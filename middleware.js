 ...
 *
 * Только на страницах с языковым адресом и только на бесперфиксных. С TRIP-533
 * это ВСЯ зона-как-страницы, включая вход и юр-документы, — переезд поехал за
 * расширением сам, второго механизма заводить не пришлось. Не переезжают
 * одноразовые capability-адреса (`/public/trip`, `/join`): языкового адреса у
 * них нет намеренно — язык такой ссылки принадлежит получателю, — и там `?lang=`

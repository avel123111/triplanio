...
export const DOORS = {
  ...
  // Два входа: контакт-id из СВОЕГО письма (он и есть секрет — id генерит Resend,
  // в приложении он не появляется нигде) ИЛИ собственная сессия из аккаунта
  // (личность из проверенного токена, не из тела). Записан по СЛАБЕЙШЕМУ входу:
  // назвав функцию `self`, карта спрятала бы тот, что рискованнее (TRIP-513).
  emailPrefs:            'token',

...
// Текстовый узел JSX: то, что стоит МЕЖДУ закрывающей скобкой тега и НАЧАЛОМ
// СЛЕДУЮЩЕГО ТЕГА. Оба конца названы точно, и оба — по живым ложным срабатываниям:
//
//   · `(?<!=)` слева: стрелка `=>` даёт регулярке тот же `>`, и обычная
//     стрелочная функция, возвращающая JSX через тернарник
//     (`=> (cond ? <A/> : <B/>)`), читалась как «захардкоженный текст "(cond ?"»;
//   · `(?=[A-Za-z/!>])` справа: закрывать текстовый узел имеет право только
//     НАЧАЛО ТЕГА (`<div`, `</div`, `<!--`, фрагмент `<>`). Без этого условия
//     пара операторов сравнения в обычном коде — `if (n >= 2 && n <= 4)` —
//     читалась как текстовый узел `= 2 && mod10`, то есть гард требовал `t()`
//     от арифметики. Глушить такое `i18n-ignore` нельзя: маркер означает
//     «строка намеренно без t()», а строки там нет.
const TEXT_NODE_RE = /(?<!=)>([^<>{}]+)<(?=[A-Za-z/!>])/g;

...
test('★ ?lang= на переведённой странице уводит на её языковой адрес', () => {
  ...
  //
  // ★ РАСШИРЕНИЕ МОДЕЛИ ПРИЕХАЛО СЮДА САМО (TRIP-533). Языковой адрес получила
  // вся зона-как-страницы, и переезд поехал за ним, потому что спрашивает он
  // ровно `LOCALISED_PAGES` — второго механизма заводить не пришлось, а список
  // в этом цикле перечислять не надо: `/login`, `/reset-password`, `/terms` и
  // `/privacy` вошли в него молча и проверяются здесь наравне с лендингом. До
  // этого рекламная ссылка на `/login?lang=es` отдавала АНГЛИЙСКИЙ документ,
  // который перерисовывался в испанский уже в браузере.
...
test('?lang= НЕ трогает capability-адреса и мусорные значения', () => {
  // У публичной поездки и приглашения языкового адреса нет НАМЕРЕННО: язык
  // такой ссылки принадлежит получателю, а не отправителю (см. LOCALISED_PAGES).
  // Там параметр работает как работал, уже в браузере.
  for (const p of ['/public/trip/abc?lang=ru', '/join/abc?lang=ru']) {

...
test('разбор импортов действительно видит зависимости middleware', () => {
  ...
  // Глубина: не только вход и его прямой сосед, но и следующий уровень —
  // `routePaths.js` тянет `demoPath.js`. На неглубоком обходе проверка выше
  // зеленела бы, не увидев большую часть графа.
  assert.ok(
    files.some((f) => f.startsWith('src/pages/')),
    `разбор не пошёл вглубь — виден только первый уровень: ${files.join(', ')}`,
  );
});

test('★ край не ходит в слой сборки', () => {
  // Не «нет модулей Node», а НАПРАВЛЕНИЕ зависимости. Имя оболочки понадобилось
  // и краю, и сборке — и лежало оно в сборочном модуле, то есть рантайм края
  // зависел от `scripts/build/`. Один раз это уже уронило ВЫКЛАДКУ после
  // успешной сборки (`node:fs` в графе), и тогда лечили симптом: тест на модули
  // Node. Он бы и сейчас был зелёным — сама перевёрнутая зависимость ему не
  // видна. Поэтому она пинится отдельно: имя переехало в общий словарь адресов
  // (`src/lib/routePaths.js`), а край в слой сборки больше не заходит вовсе.
  const files = [...walk(ENTRY).keys()].map((f) => relative(ROOT, f));
  const build = files.filter((f) => f.startsWith('scripts/'));
  assert.deepEqual(build, [],
    `край импортирует слой сборки — правка там способна уронить выкладку:\n  ${build.join('\n  ')}`);
});

...
for (const line of addedLines) {
  if (line.includes(`${EXEMPT}:`)) {
     ...
     * правила С КЛАССАМИ напечатанная единица — всегда ОДИН класс (ключ заводится
     * на каждый), у бесклассового — весь селектор. Поэтому многолексемная единица
     * законна ровно когда классов в ней нет (`h1 em` — ради этого и делался
     * разбор справа налево), а `.pro-up--inline .pu-body` гард не печатает
     * никогда: он печатает `.pro-up--inline` и `.pu-body` порознь, а составной
     * селектор — справа от значения, как подсказку для `visual-diff-move`.
     * ★ Без этой проверки разбор справа налево ОТКРЫВАЛ дыру ровно того класса,
     * от которого лечит TRIP-363: `unitOf` берёт первый класс, поэтому маркер
     * «про `.pu-body`» молча гасил бы правку на ПРЕДКЕ — общем классе. Нашёл
     * `code-simplifier` прогоном, не чтением: на `dev` такой маркер падал кодом 2,
     * а после правки печатал «изменение объявлено намеренным». Единица без
     * пробела не трогается — у `.a.b` поведение «первый класс» было и до этого. */
    const compound = u && /\s/.test(u) && classesOf(u).length > 0;

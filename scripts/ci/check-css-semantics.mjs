...
/* ★ ПЕРИМЕТР — ДВЕ ПАПКИ, И `public` ЗДЕСЬ НЕСУЩАЯ (TRIP-460).
 * Раньше стояло `-- src`, и `public/site.css` — единственный CSS сайтовой зоны,
 * 92 КБ — не видел ни один гард на ЗНАЧЕНИЯ: 2ac сторожит имена, 2ad ссылки,
 * 2k сырые кегли, а «было 15px, стало 12px» не ловилось ничем. Так через зону
 * прошёл мёртвый var(--fs-h4) (2k енфорсит сырые px, а var() на несуществующее
 * имя сырым px не является) и так же молча разъехались ступени шкалы.
 * Периметр — часть контракта гарда, поэтому он запинен тестом: см.
 * check-css-semantics.test.mjs, «периметр включает public/site.css». */
const PERIMETER = ['src', 'public'];

/* --------------------------- порядок каскада ------------------------------
 * Зафиксирован в main.jsx тремя строками (TRIP-339 Р9): app.css первым, экраны,
 * index.css последним — и ПОСЛЕ всего бандла идёт `public/site.css`: он
 * приезжает рантайм-тегом <link> из useSiteCss на маркетинговых роутах, поэтому
 * в зоне переопределяет всё, что совпало по имени с приложением. Дать ему ранг
 * ниже значило бы считать «итоговое объявление» по проигравшему правилу, и
 * правка в site.css стала бы невидимой, хотя файл уже в периметре (TRIP-460;
 * оба конца — против app.css и против index.css — запинены тестами).
 * Порядок здесь ПОВТОРЁН, а не выведен из графа модулей: гард обязан считать
 ...
 * если появится, файл всё равно окажется подключённым через того, кто
 * импортирует импортёра. */
const LOADER_RE = /\.(?:jsx?|tsx?|html)$/;

/* ★ ФАЙЛ, КОТОРЫЙ НИКТО НЕ ПОДКЛЮЧАЕТ, В КАСКАДЕ НЕ УЧАСТВУЕТ.
 *
 * Гард моделирует каскад «все .css периметра», и это неверно ровно для одного
 * случая — таблицы, на которую не осталось ни одной ссылки. Она не приезжает в
 * браузер, её объявления не выигрывают НИЧЕГО, но в модели гарда выигрывают:
 * `src/pages/login.css` (705 строк, мёртв с Ф6.5) держал ПОБЕДИТЕЛЯ у 472
 * ключей — `.btn-primary background`, `.field gap`, `.dot width`… То есть для
 * 472 ключей гард печатал не то значение, которое видит человек, и настоящая
 * правка в `app.css` по этим ключам прошла бы ЗЕЛЁНОЙ: победитель-то не менялся.
 * Мёртвый файл не просто мусор — он ГЛУШИЛ гард.
 *
 * Считается ОДИНАКОВО на обеих сторонах и из ОДНОГО дерева (у базы — из
 * `BASE_REF`, у HEAD — из рабочего), поэтому сравнение остаётся честным: файл,
 * мёртвый на обеих сторонах, исчезает из обеих карт и не порождает ни одного
 * расхождения. Предикат — по ИМЕНИ ФАЙЛА, а не по разбору импортов: `site.css`
 * подключается рантайм-тегом из `useSiteCss()`, а не `import`, и разбор
 * импортов похоронил бы всю сайтовую зону.
 *
 * Список НЕ ведётся руками сознательно: ручной периметр не растёт сам за
...
const addedLines = [
  git(['diff', '--unified=0', `${BASE_REF}...HEAD`, '--', ...PERIMETER]),
  git(['diff', '--unified=0', 'HEAD', '--', ...PERIMETER]),
]
  .join('\n')
  .split('\n')
 ...
 * Гард сравнивает значения ТЕКСТОМ, и в шапке про `resolveVars` написано, почему
 * разворачивать `var()` глобально нельзя: правка ОДНОГО токена зажгла бы сотню
 * его потребителей, у которых в тексте не изменилось ничего. Это верно и
 * остаётся в силе — здесь ДРУГОЙ случай, обратный по направлению:
 *
 *   текст      вычисленное   было              стало
 *   одинаков   одинаково     тишина            тишина
 *   одинаков   РАЗНОЕ        красный у токена, тишина у потребителей   ТО ЖЕ САМОЕ
 *   РАЗНЫЙ     одинаково     красный           тишина + печать
 *   РАЗНЫЙ     разное        красный           красный
 *
 * Меняется ровно третья строка, и правило не может сработать во второй: оно
 * рассматривает только ключи, УЖЕ попавшие в `changes`, то есть те, где текст
 * разошёлся. Цена, названная в шапке, не растёт ни на один ключ.
 *
 * ЗАЧЕМ. Перевод сырого значения на имя — `rgba(19,50,78,.22)` →
 * `rgba(var(--line-ink),.22)`, `#FFFFFF` → `var(--white)` — это ЦЕЛЬ эпика ДС, а
 * не смена облика: на экране не меняется ни один пиксель. Требовать маркер на
 * каждое такое объявление означало делать правильную работу дороже неправильной
 * (замер по альфа-хвосту зоны: 171 маркер на один PR за нулевую визуальную
 * разницу — та самая «стена обходов», которая растворяет гейт).
 *
 * ПОЧЕМУ ЭТО НЕ ОСЛАБЛЕНИЕ. Предикат — ТОТ ЖЕ `sameMovedValue`, которым гард уже
 * сверяет перенос, то есть второй формулы не заводится. Каждая сторона
 * разворачивается по СВОЕМУ `:root` (база — базовым, HEAD — HEAD'овым), поэтому
 * одновременная правка токена даёт РАЗНОЕ вычисленное и остаётся красной. Имя,
 * которого резолвер не знает (токен объявлен не на `:root` — например на
 * `html.site` или на классе), не разворачивается вовсе: тексты остаются разными,
 * ключ остаётся блокирующим. Отказ — в безопасную сторону.
 *
 * Ключи ПЕЧАТАЮТСЯ поимённо, а не только считаются: это самое «доверьтесь мне»
 * из трёх автоматических объявлений, и ревьюер обязан видеть, что именно прошло. */
let respelled = [];
for (const c of changes) {
...
if (respelled.length) {
  notes.push(`переспеллингов через var() объявлено автоматически (вычисленное совпадает): ${respelled.length}`);
}

/* ── Снятие `color` объявляется АВТОМАТИЧЕСКИ (TRIP-337) ─────────────────────
 * Удаление color-декларации — это ЦЕЛЬ эпика (цвет уезжает на роль-утилиту
 * .muted/… либо возвращается к унаследованному от body), а НЕ смена значения,
 * ради ловли которой 2p существует (`.checkbox gap 9→17`). Требовать точечный
 * маркер на КАЖДОЕ снятие цвета делало корректную чистку дороже её самой (замер:
 * −11 объявлений цвета Budget тянули +24 строки маркеров — файл РОС). Поэтому
 * УДАЛЕНИЕ `color` не требует маркера; СМЕНА значения и ДОБАВЛЕНИЕ по-прежнему
 * требуют (регресс тона ловится), и снятие ЛЮБОГО другого свойства
 * (раскладка/радиус/фон) — тоже (защита Г20/Г21 цела). Реальную защиту от
 * визуального регресса даёт рендер-ревью обеих тем (Г35), а не строка-маркер. */
let autoDroppedColor = 0;
for (const c of changes) {
  if (c.to === null && c.prop === 'color' && !declared.has(c.key)) {
    declared.add(c.key);
    autoDroppedColor += 1;
  }
}
if (autoDroppedColor) notes.push(`снятий color объявлено автоматически (цель эпика, не смена значения): ${autoDroppedColor}`);

/* ── ДОБАВЛЕНИЕ на ПОЛНОСТЬЮ НОВЫЙ класс-ключ — не регресс (TRIP-460 Ф6) ──────
 * 2p ловит тихую визуальную регрессию СУЩЕСТВУЮЩЕГО элемента (`.checkbox gap
 * 9→17`). У класса, которого на базе не было НИ В ОДНОМ свойстве, регрессировать
 * нечему по построению: это НОВЫЙ элемент, а не правка старого. Без этой ветки
 * `public/site.css` оказывался заморожен на добавление — первая же новая
 * страница зоны (login/join, дальше public-trip/docs) давала сотни ложных «+»,
 * а маркер на КАЖДОЕ — тот самый анти-паттерн «файл РОС», от которого гард и
 * предостерегает (см. блок про снятие color выше).
 *
 * Пропускаем ТОЛЬКО добавление (`from===null`) на юнит, которого на базе НЕ БЫЛО
 * ни в одном ключе. Продолжаем блокировать: смену значения (`from!==null`),
 * снятие не-`color`, и ДОБАВЛЕНИЕ свойства к классу, который на базе УЖЕ БЫЛ
 * (`.checkbox` дорисовали `background` — элемент сменил облик, `baseUnits` его
 * держит). Переименование класса дырой не становится: старый ключ уходит СНЯТИЕМ
 * не-color (→ блок), плюс 2n orphans держит разметку, плюс 2ae site-dup — имена
 * зоны. Единица = то же `partsOf(key).unit`, что и в остальных сверках. */
const baseUnits = new Set([...base.best.keys()].map((k) => partsOf(k).unit));
let autoNewUnit = 0;
for (const c of changes) {
  if (c.from === null && !declared.has(c.key) && !baseUnits.has(c.unit)) {
    declared.add(c.key);
    autoNewUnit += 1;
  }
}
if (autoNewUnit) notes.push(`добавлений на новые классы-ключи пропущено (новый элемент, не регресс): ${autoNewUnit}`);
...
if (respelled.length) {
  console.log('  переспеллинг через var() (вычисленное то же, маркер не нужен):');
  respelled.sort((a, b) => a.key.localeCompare(b.key)).slice(0, 40)
    .forEach((c) => console.log(`    ${fmtId(c)}: ${c.from} → ${c.to}`));
  if (respelled.length > 40) console.log(`    … и ещё ${respelled.length - 40}`);
}

...
const mkChange = (key, from, to) => ({ key, ...partsOf(key), from, to });
 ...
 * кодом 2, а не притворится работающим маркером. Голый тег источником тоже не
 * выражается — `h4` прочитается как класс `.h4`, такого класса нет, перенос
 * объяснит НОЛЬ объявлений, и правки `h4` останутся блокирующими: отказ в
 * безопасную сторону. Снос второго канона типографики (Р9) — это и не перенос,
 * а удаление, и объявляется точечным `visual-diff-exempt: h2 font-weight — …`. */
...
const fmt = (c) => `  ${fmtId(c)}: ${fmtValue(c)}`;
...
for (const mv of moves) {
  ...
  for (const c of changes) {
    ...
    if (c.to !== null || c.unit !== mv.from) continue;

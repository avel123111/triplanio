...
test('★★ токен объявлен НЕ на :root — переспеллинг остаётся КРАСНЫМ (отказ в безопасную сторону)', (t) => {
  // Резолвер знает только словарь `:root`. Имя, объявленное на классе или на
  // другом корневом селекторе, не разворачивается — тексты остаются разными, и
  // ключ продолжает блокировать. Граница названа вслух, а не подразумевается.
  const f = fixture(t, {

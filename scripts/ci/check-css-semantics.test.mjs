...
test('★ форма записи атрибута ключ не меняет — [x="a"] и [x=a] это одно состояние', (t) => {
  // Иначе переписывание кавычек читалось бы как «состояние исчезло и появилось
  // другое» — ровно тот класс дефекта, где форма записи решает ответ.
  const f = fixture(t, {
    base: { 'src/a.css': '.btn[data-state="open"] { gap: 9px; }\n' },
    head: { 'src/a.css': '.btn[data-state=open] { gap: 9px; }\n' },
  });
  const { code, out } = run(f);
  assert.equal(code, 0, out);
});

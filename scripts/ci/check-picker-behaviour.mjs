 ...
 *
 * ЗАЧЕМ. Все 1745 тестов проекта — грепы по исходникам: jsdom и testing-library
 * в зависимостях нет. Они доказывают, что код ВЫГЛЯДИТ как надо, и не доказывают,
 * что он работает. Цена этого записана: правило «закрытие снимает фокус» уехало
 * из хука к поверхности, `release` из него исчез, в `SearchSelect` вызов остался —
 * и `release()` падал TypeError на КАЖДОМ клике по пункту. Выбор валюты и языка
 * не работал вовсе. Это прошло lint, typecheck, 1744 теста и 13 гардов и доехало
 * до пользователя (Sentry TRIPLANIO-33).
 *
 * Проверяется ровно то, что грепом недоказуемо:
 *   1. тап по триггеру открывает шторку И ставит фокус в поле — контракт, от
 *      которого зависит подъём клавиатуры (фокус обязан быть В ЖЕСТЕ);
 *   2. КЛИК ПО ПУНКТУ ВЫБИРАЕТ ПУНКТ — тот самый прод-баг;
 *   3. закрытие снимает фокус в первых кадрах, а не через полсекунды — иначе
 *      клавиатура отстаёт от шторки (замерено: было 500 мс);
 *   4. за въезд поле не двигается (размах 0), а панель едет — каретке нечего
 *      догонять только при неподвижном поле.
 *
 * КАК ЗАПУСКАТЬ (как и `check:proto`, руками — в CI её нет: нужен браузер и
 * поднятый сервер):
 *   (setsid npx vite preview --port 4173 --strictPort &)
 *   npm run check:picker                      # или: --url http://127.0.0.1:5173
 *
 * Стенд — витрина `/kit/autocomplete`: там живой `<Autocomplete>` с локальным
 * мок-поиском, без сети и без логина. Это ЕДИНСТВЕННОЕ место в приложении, где
 * пикер можно потрогать анонимно, и потому единственное, где такая проверка
 * вообще возможна без учётки.
 *
 * ⚠️ ГРАНИЦА. Chromium — не WebKit. Каретку и клавиатуру он не воспроизводит:
 * это свойства отрисовки и платформы, их приёмка — глаза на устройстве. Здесь
 * проверяется то, что от движка не зависит: фокус, выбор, геометрия.
 */

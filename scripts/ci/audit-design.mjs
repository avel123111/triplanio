 ...
 *
 *  ПРЕДИКАТ: класс - ПОДЛЕЖАЩЕЕ правила, в объявлениях которого есть
 *  `display: flex|inline-flex|grid|inline-grid`. Подлежащее, а не все классы
 ...
 *
 *  ★★★ ПОДЛЕЖАЩЕЕ БЕРЁТСЯ ОТ ПОСЛЕДНЕЙ СТУПЕНИ СЕЛЕКТОРА, И ВНУТРИ НЕЁ - ПЕРВЫМ
 *  КЛАССОМ, А НЕ ПОСЛЕДНИМ КЛАССОМ ВСЕГО СЕЛЕКТОРА (`styledClass`). Разница
 *  видна только на составной ступени, и ровно там общий `styledClass` даёт
 *  СОСТОЯНИЕ вместо объекта: `.ncal-wdc-cbar.is-split {display:flex}` записался
 *  бы на `is-split`. Это не косметика - это дыра, пропускающая мутацию: имя
 *  состояния (`is-split`, `active`, `open`) носят РАЗНЫЕ объекты, все они
 *  схлопываются в ОДНУ запись множества, и тогда схлопнутый объект число не
 *  роняет (второй продолжает объявлять то же имя), а новый приватный класс с
 *  тем же состоянием его не поднимает. Печатается при этом имя, которого PR не
 *  трогал. `styledClass` НЕ правится: на нём стоят числа §3/§4, и «уточнить»
 *  их той же рукой, которой вводится измерение, - ровно ошибка §12.
 *
 *  ⚠️ ГРАНИЦА, НАЗВАННАЯ ВСЛУХ: если у последней ступени класса НЕТ
 *  (`.checkbox input`, `.seg button`, `.app-header [data-mobile-toggle]` - 9
 *  живых правил в периметре), раскладку объявляет голый тег, и записывается на
 *  БЛИЖАЙШИЙ класс-предок: снять её всё равно можно только правкой его набора
 *  правил. Альтернатива «не считать вовсе» уносит из наблюдения живые
 ...
 *  `:root`, шаг кейфрейма), не считается никуда.
 *

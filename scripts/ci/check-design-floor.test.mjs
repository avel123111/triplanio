...
/* ── седьмое число: доля «собрано из системы», храповик ВВЕРХ (TRIP-337 §1) ── */
/** Первая метрика эпика, которая может только РАСТИ. Проверяется не «считает ли
 *  она» (это `audit-design.test.mjs`), а что пол не принимает её падение за
 *  норму и что маркер обязан называть ЗНАК: `+N` на растущей метрике - это
 *  опечатка, а опечатка не должна читаться как «исключения не просили». */
...
test('★ доля «собрано из ДС» УПАЛА → красный, хотя все остальные числа не выросли', (t) => {
  ...
  const r = run(f);
  assert.equal(r.code, 1, r.out);
  // Именно строка НАРУШЕНИЯ, и вместе с ДЕЛЬТОЙ: само название метрики печатается
  // в таблице всегда, поэтому совпадение по нему прошло бы и при красноте от
  // любого другого числа, а без дельты остаётся неприпёртым знак — у растущей
  // метрики нарушение это `-1667`, и `+` тут читался бы как «доля выросла».
  assert.match(r.out, /✗ собрано из ДС \(bp\): 5000 → 3333 \(-1667\)/);
  // И подсказка обязана быть той, которую можно скопировать: гард сам же
  // отвергает `+N` на растущей метрике, так что напечатать её было бы советом
  // сделать то, за что он тут же даст выход 2.
...
test('маркер со ЗНАКОМ МИНУС покрывает согласованное падение доли', (t) => {
  const f = fixture(t, {
    base: { ...DS, 'src/S.jsx': screen('') },
...
test('маркер `-N` на УБЫВАЮЩЕЙ метрике - такая же ошибка (симметрия)', (t) => {
  const f = fixture(t, {
    base: { 'src/a.css': family(2) },

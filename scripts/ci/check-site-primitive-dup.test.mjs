...
test('★★ ФИГУРНАЯ СКОБКА В КОММЕНТАРИИ не рвёт скан — правку всё равно видно', () => {
  // Комментарии этого репозитория цитируют JSX, то есть содержат `{` и `}`.
  // Скан без гашения комментариев рассинхронизируется на первой такой скобке, и
  // ВСЁ, что ниже, становится для гарда невидимым — то есть он молча отвечает
  // «чисто». Ровно этот дефект уже стоил гарду 2n второй редакции.
  const prose = '/* было `<div className={x}>` — стало `{y}` */\n';
  assert.equal(runGit({
    appBase: `${prose}.btn { padding: 0 15px; }\n`,
    appHead: `${prose}.btn { padding: 0 15px; letter-spacing: .04em; }\n`,
    site: SITE_OK,
    markup: MARKUP_BTN,
  }), 1);
});

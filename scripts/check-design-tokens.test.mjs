 ...
 *
 * ★ ЗЕЛЁНЫЙ ТЕСТ НИЧЕГО НЕ ЗНАЧИТ, ПОКА НЕ УВИДЕЛ ЕГО КРАСНЫМ. Прогнаны восемь
 ...
 *   • public/landing.css сканируется по имени, его отсутствие роняет чтение;
...
test('public/landing.css вне периметра до подзадачи 10', (t) => {
  const f = fixture(t, {
    base: { 'src/a.css': '.a{padding:8px}\n' },
    head: { 'public/landing.css': '.seed{color:#123456}\n.l{padding:19px}\n' },
  });
  assert.equal(run(f).code, 0);
});
...
test('★ базовая сторона видит ТЕ ЖЕ пути, что и HEAD — иначе периметр расширить нельзя', (t) => {
  ...
  // всего лишь выводит landing.css из SPACING_ALLOW, а гард отдал бы все её 30
  ...
  // 10 выведет landing.css как раз оттуда — счётчик вхождений покраснел бы на

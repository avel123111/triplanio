...
Deno.serve(withHandler('getTripDetails', async (req, corsHeaders) => {
    ...
    if (wantContent) {
      ...
      // so rows appeared at different moments. The ids are already in hand here,
      // so it costs one in-datacentre query instead of a client round trip. This
      // bundle is now the ONLY profile door the front-end reads (TRIP-409). The
      // owner is a real member row (TRIP-516), so its profile resolves from the
      // members list — no separate ownerId injection (TRIP-517).

 ...
 *
 * Auth: OWNER ONLY (trips.created_by === caller). This check IS the gate, not a
 * mirror of one: since TRIP-190 Ф3c the `trips` table has a single policy left
 * (`trips_select`) and `update`/`delete` are revoked from `authenticated`
 * outright, so there is no `trips_delete` RLS policy to mirror. Every write to
 * `trips` goes through an edge function on the service role.
 *

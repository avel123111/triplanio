 ...
 * geoLocationiq
 *
 * Thin proxy to the LocationIQ APIs (managed Nominatim/OSM): address autocomplete
 * + street-address forward geocode. The LocationIQ key is a billable secret with
 * no HTTP-referrer restriction on Free, so it must never reach the browser — the
 * client calls this function and we attach `LOCATIONIQ_API_KEY` server-side.
 *
 * City search/resolve moved to the local GeoNames `search_gazetteer` RPC (TRIP-146);
 * reverse geocoding ("my city" by GPS) moved to the `nearest_cities` RPC (TRIP-226).
 * This function now serves ADDRESS geocoding only.
 *
 * POST body (one of):
 *   { action: 'autocomplete',   q, lang?, limit?, tag? }   → address autocomplete
 *   { action: 'geocodeAddress', q, lang?, priority? }      → forward geocode (street address)
 *
 * Response: { results } for each action. Normalization into the app's address
 * shape stays client-side in src/lib/geo.js. Both actions are addresses (high
 * cardinality, ~zero cross-user reuse) and are NEVER cached; their rate is held
 * by the shared token bucket instead (the `geocode_cache` table + the old
 * search/reverse city actions were retired with TRIP-146/226).
 *
 ...
 * is taken before every upstream call. Interactive callers (autocomplete) have
 * priority over background ones (booking-address resolve) via `p_min`.
 *
 * TRIP-60 observability: `geocode bucket exhausted` (our own token bucket ran dry →
 * tune cap/rate) vs `geocode upstream_429|upstream_5xx` (LocationIQ itself rejected →
 * key/tariff ceiling or provider outage). endpoint/status ride along in extra.
 */
...
// Per-request token-wait budget. Background waits patiently so batches (booking
// addresses, layovers) RESOLVE instead of going red; the budget is shared across
// a whole request, so total wall-clock stays bounded well under the edge/gateway
// limit. Interactive (autocomplete) stays snappy — it rarely waits at all because
// background yields to it (p_min). Sleeping is async I/O → no CPU cost.
...
function buildUrl(
  endpoint: string,
  apiKey: string,
  p: { q?: unknown; lang: string; limit: string; tag?: unknown },
): string {
  ...
  // search — used by geocodeAddress (street-address forward geocode).
  const params = new URLSearchParams({
...
// error (surfaced as 502). 404 = genuine no-match → [].
...
// Resolve one address lookup: token → upstream. Addresses are never cached.
async function resolveOne(
  endpoint: string,
  lang: string,
  priority: string,
  urlParams: { q?: unknown; limit: string; tag?: unknown },
  apiKey: string,
  deadline: number,
): Promise<ResolveOutcome> {
  // Take a token first (waits up to the request deadline).
...
Deno.serve(withHandler('geoLocationiq', async (req, corsHeaders) => {
    ...
    const { action, q, lang, limit, tag, priority } = body;

    const acceptLang = (typeof lang === 'string' && lang.trim()) ? lang.trim() : 'en';
    const lim = String(limit && limit > 0 ? limit : DEFAULT_LIMIT);

    // Address-only actions (city search → search_gazetteer RPC, TRIP-146; reverse →
    // nearest_cities RPC, TRIP-226). Both are uncached; only the priority differs.
    let endpoint: string;
    let defaultPrio = 'interactive';
    if (action === 'autocomplete') {
      endpoint = 'autocomplete';
    } else if (action === 'geocodeAddress') {
      endpoint = 'search'; defaultPrio = 'background';
    } else {
    ...
    const outcome = await resolveOne(endpoint, acceptLang, prio, { q, limit: lim, tag }, apiKey, deadline);
    ...
    if ('upstreamError' in outcome) {
      ...
      // in extra. The only callers left are address ops (autocomplete + geocodeAddress).

 ...
 *   4. FE   `EDITOR_ROLES` (src/lib/tripStep.js) — кому рисуются кнопки
 * Пропустишь половину — половина экрана работает, половина отдаёт 403.
 *
 * CHECK тут НЕСУЩИЙ: фронт (4) с TRIP-274 Ф2.1 — тоже БЕЛЫЙ список, дословное
 * зеркало этого файла (`EDITOR_ROLES` + `clearsStep`). Заведёшь четвёртую роль
 * в (1) и (2), но не в (3)/(4) — edge и фронт отдадут 403 там, где RLS пустит.
 *
 * Расхождение (2)↔(3) ловит LIVE-ассерт `checkEditorRoles`
 * (scripts/ci/check-security-tiers.mjs, гард 2e): он сверяет ЖИВОЕ тело
 * `_can_edit_trip` с этим списком на КАЖДОМ выкате. Пару (3)↔(4) ловит
 * FE-тест `src/lib/tripStep.test.js` — читает ЭТОТ файл как текст и сверяет
 * `EDITOR_ROLES` и порядок лестницы. Пары (1) в машине нет.
 */

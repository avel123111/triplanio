...
/**
 * Кэп «короткой строки» = домен `public.short_text` (`char_length(value) <= 300`).
 * Живёт ЗДЕСЬ, а не у вызывателя: у каждой границы доверия он один и тот же, и
 * второе число для одного смысла в проекте не заводится. Спеки ресурсов пишут
 * `max: 300` в колонке — там кэп зеркалит КОНКРЕТНЫЙ CHECK своей таблицы; это
 * имя — для входов БЕЗ своего хранилища (драфт планировщика, запрос агента),
 * которым потолок всё равно нужен, а зеркалить нечего.
 */
export const SHORT_TEXT = 300;

/**
 * ГОРОД, КАК ЕГО НАЗЫВАЕТ МОДЕЛЬ. Одна спека на все границы, куда ИИ приносит
 * город словами: узел драфта (`planTripWithAi/draft.ts`) и запрос инструмента
 * справочника (`gazetteerSearch/request.ts`). Раньше тройка была объявлена
 * дважды с одинаковыми кэпами — ровно тот класс, что уже стоил `nights`,
 * пропавшего из схемы парсера при переименовании типа (TRIP-527).
 *
 * `required` НЕ здесь: у драфта имя города необязательно (узел может быть
 * заготовкой), у поиска — обязательно (искать нечего). Обязательность — свойство
 * ГРАНИЦЫ, а не поля, и вызыватель дописывает её сам.
 */
export const CITY_NAME_FIELDS: Record<string, FieldSpec> = {
  city_name: { type: 'string', max: SHORT_TEXT },
  city_name_en: { type: 'string', max: SHORT_TEXT },
  // ISO 3166-1 alpha-2 — РОВНО две буквы, а не «строка не длиннее двух»: кэп
  // пропускал `U` и `1;`, а односимвольный код молча уводит скоуп в пустоту
  // (кандидатов нет → город «не разрешён»), то есть ошибка выглядит как «города
  // не существует». Проверка формы дешевле такого разбирательства.
  country_code: {
    type: 'string',
    max: 2,
    validate: (v) => (/^[A-Za-z]{2}$/.test(String(v)) ? null : bad('Field "country_code" must be an ISO 3166-1 alpha-2 code')),
  },
};

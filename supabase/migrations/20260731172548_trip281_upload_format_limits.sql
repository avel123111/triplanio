...
--
--     Тип части в multipart проставляет БРАУЗЕР по MIME-таблице ОС, а она разная
--     на разных машинах (там, где не зарегистрирован Excel, `.csv` приезжает как
--     `text/plain`). Поэтому загрузка документов пересобирает файл с типом ПО
--     РАСШИРЕНИЮ (`uploadContentType`) — так этот список не может разойтись со
--     списком расширений во фронте.
--
--     `application/octet-stream` в списке НАМЕРЕННО: три поверхности грузят в обход
--     того шва (распознавание брони, обложка, аватар), а когда браузер не знает тип
--     (HEIC в ряде десктопных браузеров), он шлёт именно его — без этого пункта
--     отвалились бы легальные фото. Небезопасным это не делает: octet-stream
--     браузер всегда скачивает, а в `<img>` определяет формат по байтам.
--
...
-- прогоняет её через normalizeExternalUrl, но так было НЕ ВСЕГДА: до TRIP-230
-- значение писалось как есть, поэтому остались строки без схемы («укпукп»).
-- Прод чист, в dev таких 7 — первый заход этой миграции на них и упал (23514).
--
-- Нормализуем ровно тем же правилом, что и живой писатель: дописываем `https://`
-- ПРЕФИКСОМ. Не чиним ссылку (она и не была рабочей) — приводим к форме, в
-- которой сохранён и текст пользователя, и инвариант схемы. Обнулять нельзя:
-- на проде это тихо стёрло бы чужие данные, если такая строка когда-нибудь
-- появится. Идемпотентно: после апдейта предикат WHERE не выбирает ничего.
--
-- `left(…, 2040)` — не косметика: на колонке уже висит `td_link_url_len` (кэп 2048,
-- TRIP-169), и приписка 8 символов к строке длиной 2041–2048 уронила бы миграцию
-- на ЭТОМ констрейнте — ровно тот же класс отказа, который тут и чинится (ревью
-- Codex к PR #624). Для любой реальной строки срез не срабатывает; 2041 символ без
-- схемы — не ссылка, а мусор, и его хвост ничего не стоит.
-- Связывает этот срез только ПРОД: в журнале dev файл уже лежит, там отработала
-- версия без него (те 7 строк были короткие, данные не разошлись).
UPDATE public.trip_documents
   SET link_url = 'https://' || left(link_url, 2040)

-- TRIP-145: persist per-segment endpoint coordinates for layover (multi-leg)
-- transfers. The `transfers` table already has from/to_latitude/longitude
-- (single-leg transfers write them); this only teaches add_layover_transfer to
-- read them from each segment's JSON and insert them. Additive + backward
-- compatible: an older client that omits the keys inserts NULL (today's behavior).
-- Coords are set client-side only on a house-level geocode match; otherwise the
-- address stays as text with no point. Original function: 0029_add_layover_transfer.sql.

...
- **Free** — без подписки. Лимит активных трипов **временно снят** (TRIP-503): порог поднят 1→1000 в предикате `public.can_create_trip` и в `src/lib/limits.js` (`FREE_ACTIVE_TRIP_LIMIT`). Штатное значение тарифа — **1 активный трип**; порог 1000 = «эффективно без лимита», путь возврата (два числа + возврат «безлимит»-копии) описан в TRIP-503. Пока порог 1000: баннер `/trips`, полноэкранный блокер планнера, `TripLimitDialog` и гейт копирования не срабатывают по построению, а различие «безлимит активных трипов» на `/pro` фактически неактивно (строка 1 обеих колонок = «Безлимит путешествий», ✓ и у Free, и у Pro). Механика подсчёта и источники — [[triplanio-trip-limit-sources]]. «Активный» = трип не полностью в прошлом (последний день ≥ сегодня); прошедшие трипы слот освобождают. Доступны все базовые линзы; pro-фичи закрыты.
...
## Ценообразование и упаковка (TRIP-181, утверждено Ильёй 2026-07-03)
Публичный ценовой якорь и граница Free/Pro. Обосновано product marketing.docx (§7), VOC-исследованием и brand voice; сверено с этой Pro-моделью.

**Value metric:** НЕ per-seat (приглашённые бесплатны = виральная петля, seat-цена её убьёт). Ценность масштабируется по числу активных трипов + power-фичам. Упаковка = аккаунт-подписка (безлимит трипов + Pro во всех своих трипах) + разовый per-trip анлок (`pro_trip`). Это ровно две оси ниже.

**Публичный якорь Pro (v1, к списанию через Stripe в USD/EUR; ₽ - витринный):**
- Годовой (ГЕРОЙ, единственный выделенный «Recommended»): **$39.99/год** (≈ ₽3 490/год). Уровень Wanderlog, ниже TripIt $49; годовая скидка ~52% vs месячный - сознательно агрессивная, гасит churn-оптику редкочастотного travel.
- Месячный: **$6.99/мес** (≈ ₽590/мес). Держим дороже годового в пересчёте, чтобы годовой был очевидно выгоден.
- Pro для одного трипа (`pro_trip`, разово, бессрочно, открывает Pro всем участникам трипа): **$8.99/трип** (≈ ₽790). Под порогом $9 = импульсная покупка; low-commitment вход для редкочастотного путешественника.

**★ФАКТ ПРОДА: списание идёт в EUR, а якорь выше сформулирован в USD (сверено 2026-08-31, TRIP-497).** Живой каталог (`provider_price`, `provider_env='live'`) и Stripe-аккаунт Triplanio держат ТЕ ЖЕ ЧИСЛА в евро: **€6.99/мес, €39.99/год, €8.99/трип** (`account_pro_monthly` / `account_pro_yearly` / `trip_pro_lifetime`). Цену на экране рисует не константа, а `getStripePrices` → каталог → Stripe, поэтому покупатель видит евро. Следствие для позиционирования: сравнение «уровень Wanderlog, ниже TripIt $49» считалось в долларах, а платят евро — при курсе евро выше доллара фактическая цена ВЫШЕ якоря, и конкурентную рамку надо пересчитывать по текущему курсу, а не читать числа как долларовые. Публичные тексты (`public/llms.txt`) называют цену в EUR — по каталогу, а не по этому документу.

**Упаковка (важно для recurring-дохода):** точка каннибализации «трип vs годовой» = ~4 покупки ($8.99×4 < $39.99), а сегменты делают 1-4 поездки/год → большинство рационально берёт per-trip. Для редкочастотного travel это ОК (монетизируем тех, кто на подписку не подпишется, + партнёрка добирает free). Чтобы recurring не проседал: годовой - единственный герой на публичной ценовой странице; `pro_trip` НЕ co-равная колонка на лендинге, а контекстный оффер в in-trip момент апгрейда (paywall внутри трипа) и владельцу группового трипа (`Pro.jsx?hidePerTrip=1`).

**Граница Free/Pro (утверждена TRIP-181):**
- **Free** (штатно 1 активный трип; сейчас лимит временно снят — порог 1000, TRIP-503, см. строку Free выше): все базовые линзы (таймлайн, карта, документы, календарь, подбор отелей/активностей); полная коллаборация (безлимит приглашённых, real-time, шеринг-ссылка) - НЕ гейтить, это виральное топливо (VOC тема K: рынок ненавидит платный шеринг); ручные брони; «Моя статистика»; **базовые in-app уведомления, включая базовый алерт о бесплатной отмене** (доставка in-app).
- **Pro** (подписка ИЛИ pro_trip): безлимит активных трипов + во всех своих трипах: AI-распознавание броней (любой файл); Telegram-напоминания + AI-помощник в Telegram + **доставка алертов о бесплатной отмене в Telegram**; **бюджет-линза/сплит расходов - ПОЛНОСТЬЮ за Pro** (решение Ильи; системные траты всё равно пишутся всегда, см. ниже); in-app чат с @assistant.
- ⚠️ In-app доставка уведомлений (базовый алерт отмены во Free) может потребовать доработки - текущая инфра напоминаний Telegram-центрична (getPendingReminders/getDailyReminders).

**Freemium-логика:** постоянный free-tier, БЕЗ скрытого триала (VOC тема K + brand voice «не прячем free за скрытым триалом»). Партнёрка (affiliate) монетизирует free/one-trip сегмент - подписка и комиссия не конкурируют. Опционально (эксперимент, не v1-дефолт): reverse-trial (первый трип с Pro-фичами → даунгрейд до Free) для доведения до «вау» (AI заполнил карточку).

**Открытые флаги (вне цены):**
- **WTP RU-рынка НЕ валидирован** (Gap-анализ VOC прямо это помечает; ценность Telegram-напоминаний/бота - самая непроверенная гипотеза). Якорь v1 = гипотеза; валидировать планом из VOC (5-8 интервью/сегмент + Telegram-поллы) до фиксации «навсегда».
- **Stripe не сеттлит RUB и не принимает РФ-карты**, а RU/СНГ - PRIMARY-рынок → нужен отдельный платёжный рельс, иначе основной сегмент не оплатит Pro. Отдельная GTM-задача (важнее самой цифры). В коде цена рендерится из Stripe (`getStripePrices`→unit_amount/currency); RUB в `currencies.js` - это валюта бюджета трипа, НЕ подписки.
...
- Создание 2+ активного трипа — штатно только при активной подписке (сейчас лимит временно снят, порог 1000 — TRIP-503).
...
- Предикат `public.can_create_trip` (зовут триггер `trips_enforce_limit`, шов записи `trip_quota`, `create_trip_with_route`, `copy_trip`): free = 1 активный трип, иначе `TRIP_LIMIT_REACHED`; сейчас порог временно 1000 (TRIP-503). RPC `create_trip` дропнут в TRIP-406 (миграция `20260812190642`) — авторитет давно `can_create_trip`.

...
**CORS allow-list.** Раньше все функции отдавали `Access-Control-Allow-Origin: '*'` (общий `_shared/cors.ts` + 9 инлайн-копий). Теперь единый источник — `_shared/cors.ts`, функция `corsFor(req)`: отражает `Origin` только если он в allow-list, иначе ACAO не отдаётся. Все ~45 функций переведены на shadow-паттерн: первая строка хендлера `const corsHeaders = corsFor(req);` (существующие `headers: corsHeaders` не трогались — минимальный диф). Сервер-серверные хелперы (`n8nAuth.ts`, `tripPayload.ts`) зовут `corsFor()` без аргумента (s2s Origin не шлют → CORS им безразличен; их гейт = Bearer/N8N_SECRET). Allow-list: `triplanio.com`, `www.triplanio.com`, `dev.triplanio.com`, превью Vercel (regex), `localhost:5173/3000`. ⚠️ **ГРАБЛЯ (фикс PR #190, 2026-06-26):** у Vercel ДВА формата превью — сырой deployment-URL `triplanio-<hash>-…vercel.app` (слаг проекта = `triplanio`, БЕЗ `app`; именно его открывает дашборд) И branch-алиас `triplanioapp-git-<branch>-…vercel.app`. Изначальный regex `triplanioapp-…` ловил только алиасы → ВСЕ сырые превью на dev ловили CORS-отказ (Pavel не мог войти в трип). Исправлено: сегмент `app` опционален — `/^https:\/\/triplanio(?:app)?-[a-z0-9-]+-avel123111-5277s-projects\.vercel\.app$/`. P3: не в паре с Allow-Credentials (auth=Bearer-хедер, не cookie) → защита-в-глубину от replay украденного токена, не жёсткий гейт. CAPTCHA **сознательно НЕ ставили** (видима юзеру) — резерв на GA.

...
★ **Назначение (заведено TRIP-102, 2026-06-26).** Сюда складываем Supabase-специфичные вещи, которые при гипотетической миграции на другой бэкенд нельзя перенести «как есть» — чтобы не потерять контекст. Pavel хочет зеркало этого в Notion (агенту Notion из Cyrus-сессии НЕ доступен — текст для вставки даёт в треде; эта заметка = резерв).

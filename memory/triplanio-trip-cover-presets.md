...
## Смежное: фоны share-КАРТОЧКИ — свой бакет БЕЗ таблицы (`card-bg-presets`)
Не путать с обложками трипа. Фоны конструктора share-карточки живут в отдельном
публичном бакете **`card-bg-presets`** (миграция `20260824164218`), и таблицы-каталога
у них НЕТ намеренно: на фон никто не ссылается персистентно (карточка растеризуется
в PNG в момент генерации, URL нигде не хранится), поэтому: формат = ПАПКА
`story/` либо `post/` (у 9:16 и 4:5 разный арт — ось одной коллекции, не второй
бакет; файлы в корне в витрину не попадают), порядок = префикс имени файла
(`01-…`), снятие с витрины = удаление файла, id не нужен. Overlay отдаёт фоны
ЗАПРОШЕННОГО format; смена формата в конструкторе откатывает чужой пресет на
«Стандарт» (своё фото переживает — slice-кроп). Листает бакет ТОЛЬКО
edge `render-share-card` под service_role и отдаёт публичные URL полем `backgrounds`
overlay-ответа (дверь экрана конструктора, TRIP-374); политик на бакете ноль
(TRIP-48), mime-белый список без svg (TRIP-281). Кураторство: залить файлы в бакет —
всё, никаких insert'ов. Своё фото юзера как фон карточки НЕ сохраняется никуда —
эфемерный data-URI в памяти вкладки (осознанно).

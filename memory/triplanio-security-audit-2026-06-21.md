...
**P1 — `syncTripExpense`: ★ЗАКРЫТ (TRIP-45, 2026-07-04, PR в dev).** Оказалась полностью мёртвой: 0 коллеров в репозитории (фронт пишет `budget_expenses` напрямую), 0 в n8n (перепроверены все 14 воркфлоу — каждый исходящий HTTP-URL идентифицирован), автосинк расходов делает DB-триггер `sync_budget_expense()` на hotel_stays/activities/transfers/trip_services (baseline, `trg_sync_budget_*`) — он под `is_trip_participant` RLS, HTTP-поверхности не имеет, риска нет. Удалена (Pavel одобрил): выпилены папка функции + блок в `config.toml` + запись в pinned-false наборе (13→12). ⚠️ Репо-PR НЕ снимает живую функцию с Supabase (`functions deploy` не прунит) — живую v21 на dev+prod должен снести Pavel вручную `supabase functions delete syncTripExpense --project-ref <dev|prod>`. Была ВООБЩЕ без auth (verify_jwt=false, ACTIVE): неаутентиф. запись + нескоупленный `budget_expenses.delete().eq('source_id',…)`. Дублирует триггеры 0006. Фикс: requireN8nSecret + скоуп delete, или удалить (спросить Pavel).

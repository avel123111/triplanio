...
Баг: в чат-композере (ChatLens.jsx + ChatWidget.jsx, общие классы `.chat-ov`/`.chat-ta`, `highlightMentions` — единый экспорт в `src/lib/triplanio.js`, до TRIP-296 был копипастой в обоих файлах) каретка уходила далеко вперёд от видимого текста — **только на узкой ширине (≤640px), с тегом и без**.

⚠️ Не ставить `font-weight` на подсветку упоминания В ОВЕРЛЕЕ: жирность меняет advance width и возвращает дрейф. Болд-вид даёт `-webkit-text-stroke`. В отрендеренном сообщении (не в композере) вес менять безопасно.

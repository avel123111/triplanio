...
Пунктуация в HTML одна на все языки, отсюда две цены: испанский `¡` уехал в ЗНАЧЕНИЕ ключа
(`common.email_greeting` = `¡Hola`, иначе его некуда поставить), а английское `Hi, Pavel!` несёт
запятую, которой там обычно нет. Пустое `full_name` даёт «Привет, !» — принято осознанно (решение
Pavel, как в invite-лейне), ветвления во флоу ради этого не заводим.

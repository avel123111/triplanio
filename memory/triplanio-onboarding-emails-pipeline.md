...
поля со строкой. Отсюда три выражения онбординга:
```
greeting = {{ t[lang].email_greeting }}, {{ first_name }}!      → «Привет, Павел!»
subject  = {{ t[lang].email_subject }} «{{ trip_name }}»        → «Поездка ждёт продолжения «Париж и Рим»»
intro    = «{{ trip_name }}» {{ t[lang].email_intro }}          → «„Париж и Рим“ сохранена и ждёт…»
```
Пунктуация (запятая, `!`, «») живёт в ВЫРАЖЕНИИ, одна на все языки — поэтому позиция имени фиксирована
по построению. Исключение: испанский `¡` уехал в ЗНАЧЕНИЕ ключа (`common.email_greeting` = `¡Hola`),
иначе его некуда поставить.

**Пустое `full_name` не обрабатывается — решение Pavel**, как в invite-лейне (там `{{ full_name }}
{{ t.email_intro }}` при пустом имени даёт лишний пробел). Даст «Привет, !» — принято осознанно,
ветвления во флоу ради этого не заводим.

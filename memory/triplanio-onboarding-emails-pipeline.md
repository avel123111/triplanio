...
## Лейн догонялок: `Onboarding reminders (cron)` (`GDhh5KKPJV2Uemih`)

ОТДЕЛЬНЫЙ воркфлоу (не `Communications (per-event)`: там вход — вебхук события, здесь — время).
Неактивен, отправки в нём НЕТ — цепочка кончается двумя Set-нодами:

```
Reminders daily (Schedule, 10:00)
...
= столько же писем одному человеку, сколько запусков. Идемпотентности отправки тут ещё нет.

**«Есть трип» = свой трип (`trips.created_by`).** Участник ЧУЖОГО трипа попадёт в ветку «нет трипа»
— открытый вопрос к тексту письма. Замер прода 04.09.2026: за 30 дней 30 регистраций, 12 с трипом,
18 без, «только участник» 0.

`env` зашит `production` — лейн ходит только в `Triplanio main`; dev-лейн, если понадобится, это
копия ветки с кредом `Triplanio dev` (как у welcome, где две PG-ноды).

...
- **Встроенные `$initial_utm_*` PostHog у нас МЁРТВЫЕ - на них нельзя строить
  ни один отчёт.** Проверено на проде 03.08.2026: у ВСЕХ 24 персон
  `$initial_utm_source/_medium/_campaign` = NULL, при этом `$initial_current_url`
  заполнен - и он равен `/login`, `/settings`, `/trips`, `/trip/*/edit`. То есть
  «initial» у PostHog = страница, где PostHog **запустился**, а не страница, куда
  человек **пришёл**. Причина структурная и наша: по [[triplanio-cookie-consent]]
  `posthog.init()` происходит только после согласия, а к этому моменту utm из
  адреса давно ушли. Пока стоит consent-gate, first-touch от PostHog получить
  нельзя в принципе - это цена решения по cookie, не баг.

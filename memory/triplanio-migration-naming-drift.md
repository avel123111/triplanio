...
- ~~baseline = catalog-generated~~ → **TRIP-102 ЗАКРЫТО 2026-06-26 (PR #176→dev):** baseline `20260625120000` заменён на **настоящий `supabase db dump` схемы public с prod** (Pavel снял локально через session-pooler+Docker — у агента нет Docker/CLI/pg_dump/prod-URL). Исправлено: 34 литеральных `\n`-артефакта в `CREATE TABLE` (старый снимок не поднимался на чистой БД), вью теперь в топопорядке (6 вью `ai_cost_by_*` всё равно независимы — читают базовую `ai_usage_events`). Добавлен идемпотентный **storage-блок** в конце файла (бакеты `avatars`/`trips` + 12 RLS-политик `storage.objects`, guard на `to_regclass`). Платформенные auth/storage DDL и `storage.protect_delete` НЕ включены (создаёт сам стек, иначе коллизия). Версия-таймстамп сохранена → `db push` no-op, журналы prod/dev/репо синхронны. ⚠️ Верификацию `supabase db reset` на чистой БД делает Pavel (агенту Docker недоступен) — до зелёного прогона PR не мёрджить. Тех-долг: 6 из 12 storage-политик мертвы (bucket_id `trip-covers`/`documents` без бакетов). Будущая миграция бэкенда: storage.buckets/objects — Supabase-специфичны, см. [[triplanio-backend-migration-notes]].

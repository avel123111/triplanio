...
- **ManualPlanner.jsx** ГОТОВ (/new-trip+/plan-trip-ai): новая секция `planner` (**83 ключа** в ru/en/es + import/спред в index 3 языков). STEPS→labelKey; своя дата-функция экрана переписана была на Intl, а с TRIP-527 снята совсем — формат даты идёт общей дверью `formatDayMonth`/`fmtDate` (убраны RU _MONTHS_SHORT); computeAutoTitle получил t; 7 под-компонентов на useT; CityRow получил lang. ⚠ НЕ переиспользовал ai_plan.save_trip ('Сохранить Поездку') — глоссарий-долг (см. ниже).
...
**Относительное время («5 минут назад») — `Intl.RelativeTimeFormat`, НЕ luxon (TRIP-312, 2026-07-27).** Живёт в шве `src/lib/i18n/format.js` как `formatRelativeTime`, наружу через `useI18nFormat().fmtRelative`; два потребителя — колокольчик (`NotificationsBell`) и «Входящие» (`Inbox`). До этого был `date-fns/formatDistanceToNowStrict`, а карта `DATE_LOCALES` + вычисление `dateLocale` были СКОПИРОВАНЫ в оба файла и прокидывались пропом в строку — это и был настоящий дубль, а не сам пакет. **Грабли: не «упрощать» обратно на luxon.** `DateTime.toRelative()` ТРУНКИРУЕТ, а не округляет: 90 минут → «1 час назад», 36 часов → «1 день назад» (потеря информации, видимая регрессия). Наша реализация берёт крупнейшую подходящую единицу, округляет к ближайшему (месяц = 30 дней, год = 365) и делает **перенос разряда**, которого нет ни у luxon, ни у date-fns: 3575с → «1 час назад», а не «60 минут назад». Сверено с date-fns на 12 600 точках (ru/en/es): 18 расхождений (0.14%), все на границах единиц и все в лучшую сторону. Юнит-теста НЕТ и добавить его нельзя без реструктуризации: `format.js` делает `import './translations'` без расширения — резолвит только Vite, голый `node --test` падает (та же причина, по которой `trip-cities.js` держат чистым от i18n/luxon-импортов).

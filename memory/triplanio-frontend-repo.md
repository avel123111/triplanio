...
**Бюджет — системные траты эвентов:** синк бюджета делает SQL-триггер `sync_budget_expense()` (на hotel_stays/transfers/activities/trip_services). Автосинк работает даже для не-Pro и при выключенном аддоне бюджета — это продуктовое решение (триггер БД, не гейтится). Легаси edge `syncTripExpense` (дублировала триггер, клиент её не звал, unauth verify_jwt=false) УДАЛЕНА в TRIP-45 2026-07-04 — см. [[triplanio-security-audit-2026-06-21]]. Баг (пофикшен 2026-05-30, миграция 0010): при price null/0 трата удалялась; теперь удаляется только при удалении эвента (TG_OP=DELETE), иначе апсерт с `coalesce(price,0)`. Применено prod+dev. Уже удалённые ранее траты не возвращаются сами — нужен ре-сейв эвента или бэкфилл. Связано: [[triplanio-ai-booking-parse]].

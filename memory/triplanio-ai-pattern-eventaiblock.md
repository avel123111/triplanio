...
Все AI-кнопки идут через общий `<Btn variant="ai">` → `.btn--ai` (включая `PanelAi.jsx` create-flow — прежний инлайн-дубль `aiBtnStyle()` не существует). `AiField`/`.field--ai` и чат-AI (A5) каноничны. Notion «AI Features» → раздел «UI / состояния (EventAiBlock)» обновлён. Связано: [[triplanio-overlay-pro-unification]] [[triplanio-editor-panels-redesign]]

...
| TR_NOT_ADJACENT | W | structure | from/to не соседние узлы | — | Маршрут не сходится: {from} → {to} не соседние | (Q3 редизайн редактора 2026-06-04: понижено E→W — переезд «вне плана», не блокирует сохранение, показывается в трее «вне плана») |
...
> **НЕ добавлять правило «город прилёта пересадки ≠ город вылета следующего сегмента».** Оно недостижимо по конструкции: город вылета сегмента i+1 — это read-only зеркало `segs[i].toCity` (один объект, отдельного поля нет; `SegmentsEditor` стр. 1846/1867, сохранение `saveLayoverChain` по тем же узлам). Несогласованность предотвращается на вводе, а не валидируется постфактум — это правильнее, чем ошибка. Адреса (`from_address`/`to_address`) намеренно свободные и независимые (аэропорт↔вокзал в одном городе) — не валидируем.
...
## 12. AI-парсинг = недоверенный черновик (через стандартный гейт)

**Принцип (решение Pavel):** НЕ городить отдельные проверки на AI-ввод. Распарсенные данные — это обычный черновик формы. Мапим их в поля **как есть** и прогоняем через тот же `validateEntity`; **поля, не прошедшие валидацию, НЕ скипаем — заполняем и показываем стандартные варнинги/ошибки** (те же `validation.<CODE>`, что и при ручном вводе). Пользователь сразу видит, что именно AI заполнил неверно, и правит.

**Что убираем из текущих хендлеров** (`handleHotelExtract`/`handleTransferExtract`):
- `setIf` со скипом пустых/`null` → **всегда** кладём значение (пустое поле потом ловит `*_REQUIRED`);
- `typeof === 'number'`/`boolean` гарды на скаляры → кладём как есть, валидация/нормализация на сохранении разберётся;
- бесполезные AI-only варнинги (`aiEndpointWarn`: окно дат / OOB) → их роль берут **стандартные** OOB/`TR_DEP_DAY`/`TR_ARR_DAY` из движка.

**Поток:** распарсили → замапили все поля в draft → сразу `validateEntity(draft, ctx)` → `<FieldError>` + `<IssuesPanel>` подсвечивают проблемы. Кнопка сохранения заблокирована, пока есть error — ровно как при ручном вводе.

**Механические нюансы (где «положить как есть» невозможно буквально — это не нарушение принципа):**
- **Закрытые наборы** (`transport_type`, `currency`, `payment_status`): нельзя засунуть в кнопочный селектор значение вне набора. Если AI прислал значение из набора — выбираем; иначе оставляем дефолт. Без отдельного варнинга (не структурный блокер).
- **Дата/время:** `datetime-local` не может показать невалидную строку. Если AI-дата парсится в `YYYY-MM-DDTHH:mm` — кладём; иначе поле остаётся с **предзаполненным из контекста** дефолтом (инвариант «не пустое» сохраняется), помечаем как не-AI-заполненное. Пустого/битого значения в поле не будет → стандартное `*_REQUIRED` сработает только если и дефолта нет.
- **Город:** имя от AI ре-резолвим через гео-поиск (координаты/tz — из нашего справочника, не от AI). Не нашли → поле города пустое → стандартный `SEG_CITY_REQUIRED`/required. Координаты НЕ фабрикуем.

**Итог:** отдельного «AI-валидатора» нет — один гейт на сохранении защищает и ручной ввод, и AI. Хендлеры становятся тонкими мапперами + один вызов `validateEntity`.

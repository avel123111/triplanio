          ...
          <I18nProvider>
            <QueryClientProvider client={queryClientInstance}>
              <ConfirmProvider>
                ...
                <MapProvider>
                  {/* ★ ПЕРЕХОД — ТРАНЗИШН, А НЕ ОБЫЧНОЕ ОБНОВЛЕНИЕ. Экраны едут
                      отдельными чанками (TRIP-445); без транзишна первый вход в
                      чанк за сессию ронял текущий экран в fallback Suspense — а
                      это <AppLoading>, то есть ЗАСТАВКА ЗАПУСКА посреди работы
                      («вход в трип после создания перезагружает страницу»). В
                      транзишне React держит старый экран, пока новый не готов,
                      и меняет их одним коммитом: карта-синглтон переезжает между
                      слотами без пересоздания, заставки нет. Флаг v7 роутера —
                      именно это поведение, своего кода перехода ноль. */}

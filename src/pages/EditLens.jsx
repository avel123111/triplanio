  ...
  const steps = (
    <>
      {/* Шаг 1 — город. Выбор заполняет слот (флаг+имя+«Изменить»), не добавляя
          сразу; это открывает шаг типа ниже. `autoFocus` на телефоне значит не
          «каретка», а «распахни пикер» — здесь это неверно: композер только что
          открылся, и накрывать его сверху раньше, чем человек его увидел, значит
          показать ему шторку без контекста. Ровно тот же явный отказ стоит у
          окна «добавить место» в статистике. */}
      {!city ? (

...
// Там же исправлено УТВЕРЖДЕНИЕ, которое было неверным и до этой задачи: «до
// ответа на баннер PostHog не собирает НИЧЕГО» верно только для браузера —
// серверные события (`captureServer`) уходят независимо от баннера, на
// легитимном интересе. В тексте они названы КАТЕГОРИЕЙ, а не списком: перечень
// конкретных событий — это состояние кода, которое разъедется с документом на
// первом же новом вызове, и место ему в памяти, а не в политике.
...
// 1.4 (та же дата) — правка ТОНА, не объёма обработки: из обоих документов убрано
// описание РЕАЛИЗАЦИИ (селекторы маскировки реплея, алгоритм хеша, регион
// провайдера, 60-секундный кулдаун, «billing portal», перечень AI-фич) и
// самооценочные вставки («what actually happens», «nothing we cannot explain»).
// Причина не стилистическая: реализация меняется без правки документа, и каждая
// такая фраза — обязательство, которое тихо становится ложным. Обязательства
// остались, описания механизмов ушли. Заодно выровнено уведомление об изменениях:
// Terms обещали жёсткие 14 дней, Privacy — уже нет; теперь формулировка одна.

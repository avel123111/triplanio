...
// floor-exempt: dsshare +15 — ЗАМЕР НА origin/dev, А НЕ ОЦЕНКА (пересчитан
// после ребейза): доля 42.78% → 42.64%, то есть 4278 → 4264 bp, просадка 14 при
// запрошенном потолке 15. Числитель 1439 → 1426, знаменатель 3364 → 3344 — то
// есть УПАЛИ ОБА, разметки стало меньше. Причин четыре, и все — известные слепые
// зоны метрики, а не сырая разметка. Поштучные bp здесь НЕ приводятся намеренно:
// разделить 14 на четыре причины можно только четырьмя отдельными замерами, и
// цифра «−3bp у второй причины» без такого замера была бы оценкой, выданной за
// факт (ровно этим и разошёлся прежний текст маркера с гардом).
//   (1) Разметка УДАЛЕНА, не добавлена. Режим редактирования в ряду (своё поле
//       города, «✓», staged) и инлайн-пикер в плитке старта были дублями общего
//       композера; удалённые дубли оказались плотнее по ДС, чем среднее по
//       репозиторию, поэтому доля падает оттого, что дублей стало меньше.
//   (2) Композер на телефоне сменил примитив `Sheet` (ДС) на `PickerSheet` —
//       метрика считает `components/ui/` ЛЕГАСИ: в знаменателе, но не в
//       числителе. Тот же капкан описан в маркере PR #1086.
//   (3) Якорь маршрута рисовали ДВА компонента, и второй (`GridEndpoint` в
//       редакторе) был собран из восьми примитивов ДС. Он схлопнут в ОДИН вызов
//       общего `CityAnchorRow` — то есть восемь листьев ДС заменились одним
//       вызовом компонента, который живёт в `pages/create/` и в числитель не
//       идёт. Тот же счёт наказывает и снятие инлайнов у его кнопки удаления.
//   (4) Ряд «выбранный город + изменить» диалог статистики собирал ЗАНОВО
//       (карточка ДС + флаг + имя, четыре листа) — теперь это один вызов общего
//       `cities/SelectedCity`. Тот же счёт: дубль снят, числитель просел.
// Направление лечения общее для (2), (3) и (4): переезд общих поверхностей и
// рядов в `src/design/` поднимет долю обратно и тянет всех вызывателей —
// отдельная задача, флаг по правилу #6.
// Остальные числа пола не выросли НИ ОДНО: классов 1216 → 1215, namespace 118=,
// инлайнов 480 → 474, токенов 177=, на разборе 878 → 877.
//
...
//
// Апрув Pavel получен 31.08.2026.
// ─── CityRow ──────────────────────────────────────────────────────────────────

...
import {
  Avatar, Badge, Btn, Card, Severity, Toggle, ModalHost,
} from '../../design/index';
...
/**
 * Map getUserPlan response → one of the 4 display states used in the design:
 *   'no-sub'    — Free plan
 *   'with-sub'  — Pro monthly, active
 *   'annual'    — Pro yearly, active
 *   'cancelled' — Pro cancelled (valid until subscriptionEnd)
 */
...
export default function ScreenAccount() {
  ...
  // useSearchParams needs a Router context. DesignPreview wraps everything in
  // BrowserRouter so this is safe. In tests/storybook we fall back gracefully.
  let searchParams;
  try {
  ...
  // window.__accountState is set by the DesignPreview tweaks switcher so all
  // 4 subscription states can be demoed. In production this is never set.
  ...
  // Mirrors the exact same polling logic from base44 Settings.jsx
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Refresh AuthContext so AppHeader / UserMenu pick up the new name/avatar
      await checkUserAuth?.();
      setSavedFlash(true);
  ...
  const handleAvatarUpload = async (file) => {
    ...
    try {
      ...
      // Cache-buster so the browser doesn't show the stale avatar immediately
  ...
  const handleManageSubscription = async () => {
    // Stripe billing portal must not be opened inside an iframe
  ...
  const handleDeleteAccount = () => {
    // If user has an active (non-cancelled) Pro subscription, block deletion
    // so they must cancel via billing portal first.
  ...
  const performDeleteAccount = async () => {
    ...
    try {
      ...
      if (error) throw error;
      if (data?.error) {
        // Backend also guards active subscriptions — surface as 'blocked'
      ...
      // All data deleted — sign out and redirect to landing
      await logout();
    } catch (e) {
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto' }}>
      ...
      <Card title="Идентичность" style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 18, marginBottom: 16 }}>
          <div
            ...
            style={{ position: 'relative', width: 76, height: 76, borderRadius: '50%', overflow: 'hidden', cursor: 'pointer', flexShrink: 0 }}
          >
            <Avatar name={fullName || user.email} photo={avatarUrl || undefined} size="xl" />
            {(avatarHover || uploadingAvatar) && (
              <div style={{
                position: 'absolute', inset: 0, background: 'rgba(15,23,42,.65)',
                display: 'flex', flexDirection: 'column', alignItems: 'center',
                justifyContent: 'center', gap: 4, color: 'white', fontSize: 11, fontWeight: 600,
              }}>
                {uploadingAvatar
      ...
      {/* Upgrade dialog — existing component until WS-A sprint replaces it with ScreenPro */}

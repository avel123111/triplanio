...
import { Btn, Avatar, AvatarStack, Badge, Card, EmptyState, TRIPS } from '../../design/index';
  ...
  return (
    <div style={{ maxWidth: 720, margin: "60px auto", textAlign: "center" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, maxWidth: 600, margin: "0 auto" }}>
        ...
        <button onClick={() => {}} className="ai-card" style={{
    ...
    <div style={{ maxWidth: 1200, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 16, marginBottom: 24, flexWrap: "wrap" }}>
        ...
        <div style={{ position: "relative" }}>
          <Btn variant="primary" size="lg" icon="plus" onClick={() => setCreateMenu(!createMenu)}>Новый трип</Btn>
          {createMenu && (
            <div style={{
              position: "absolute", top: "calc(100% + 8px)", right: 0, width: 320, zIndex: 30,
              background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 14,
              boxShadow: "var(--shadow-pop)", padding: 8,
            }}>
              <button onClick={() => { setCreateMenu(false); }} style={{
                width: "100%", textAlign: "left", padding: "12px 14px", border: "none", background: "transparent",
                borderRadius: 10, cursor: "pointer", display: "flex", gap: 12, alignItems: "flex-start",
              ...
              <button onClick={() => { setCreateMenu(false); }} style={{
                width: "100%", textAlign: "left", padding: "12px 14px", border: "none", background: "transparent",
                borderRadius: 10, cursor: "pointer", display: "flex", gap: 12, alignItems: "flex-start",
              }} onMouseEnter={(e) => e.currentTarget.style.background = "var(--ai-soft)"} onMouseLeave={(e) => e.currentTarget.style.background = ""}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg, #6a3ee2, #c66ce2)", color: "white", display: "grid", placeItems: "center" }}>
                  <Icon name="sparkles" size={18} />
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 600, fontSize: 13.5, display: "flex", gap: 6, alignItems: "center" }} className="ai-text">
                    Начать с ИИ <Badge variant="warm">Pro</Badge>
                  </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 18, flexWrap: "wrap" }}>
        <div className="tweaks__seg">
          <button className={activeTab === "active" ? "active" : ""} onClick={() => setActiveTab("active")}>Активные · {active.length}</button>
          <button className={activeTab === "past" ? "active" : ""} onClick={() => setActiveTab("past")}>Прошедшие · {past.length}</button>
        </div>
        ...
        <div className="tweaks__seg" title="Вид">
          <button className={view === "A" ? "active" : ""} onClick={() => setView("A")}><Icon name="grid" size={13} /></button>
          <button className={view === "B" ? "active" : ""} onClick={() => setView("B")}><Icon name="list" size={13} /></button>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
          {filtered.map(t => <TripCard key={t.id} trip={t} onClick={() => window.__triplanioNavigate?.("timeline")} />)}
          {activeTab === "active" && (
            <button onClick={() => setCreateMenu(true)} style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {filtered.map(t => <TripRow key={t.id} trip={t} onClick={() => window.__triplanioNavigate?.("timeline")} />)}
        </div>
      )}

      {/* Free limit banner — only in active */}
      {activeTab === "active" && (
        <div className="ai-card" style={{
          ...
          <Btn variant="primary" onClick={() => window.__triplanioNavigate?.("pro")}>Перейти к Pro</Btn>
        </div>
      )}
    </div>
  );

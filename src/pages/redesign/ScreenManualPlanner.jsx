...
// =====================================================================
// MANUAL PLANNER — multi-step wizard to build a trip from scratch
// (Pavel: "Собрать руками" → home → cities → return home → review → save)
// Every step shows a live map preview on the right.
// =====================================================================
...
// Cities "near" the user's geolocation (mocked — would come from IP/GPS)
const NEARBY_HOME = [
  { city: "Москва", country: "🇷🇺 Россия", airport: "SVO", lat: 55.75, lng: 37.62, distance: "8 км · ваш город" },
  { city: "Санкт-Петербург", country: "🇷🇺 Россия", airport: "LED", lat: 59.94, lng: 30.31, distance: "640 км" },
  { city: "Минск", country: "🇧🇾 Беларусь", airport: "MSQ", lat: 53.90, lng: 27.55, distance: "700 км" },
  { city: "Хельсинки", country: "🇫🇮 Финляндия", airport: "HEL", lat: 60.17, lng: 24.94, distance: "900 км" },
];
...
// Lookup for city coords by name (covers all preset cities)
const CITY_COORDS = {
...
function ScreenManualPlanner() {
  const variant = window.__manualVariant || "home";
  const [step, setStep] = useState(variant);
  useEffect(() => { setStep(variant); }, [variant]);

  const [home, setHome] = useState({ city: "Москва", country: "🇷🇺 Россия", airport: "SVO", lat: 55.75, lng: 37.62 });
  const [cities, setCities] = useState([
    { id: 1, city: "Лиссабон", country: "🇵🇹 Португалия", startDate: "2026-07-12", nights: 4 },
    { id: 2, city: "Порту", country: "🇵🇹 Португалия", startDate: "2026-07-16", nights: 3 },
    { id: 3, city: "Барселона", country: "🇪🇸 Испания", startDate: "2026-07-19", nights: 4 },
  ]);
  ...
  const save = () => {
    setSaving(true);
    setSavedOk(false);
    setTimeout(() => { setSaving(false); setSavedOk(true); }, 1400);
  };

  const lastCity = cities[cities.length - 1]?.city || "последний город";
  const effectiveReturn = returnMode === "home" ? home : returnCity;

  // Mark map cities for highlighting based on current step
  const mapHighlight = step === "home" ? "home" : step === "return" ? "return" : step === "cities" ? "cities" : "all";

  return (
    <div style={{ maxWidth: 1280, margin: "0 auto" }}>
      {/* Top header — back + stepper */}
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 18, flexWrap: "wrap" }}>
        <Btn variant="ghost" size="sm" icon="back" onClick={() => window.__navigate?.("collection")}>К коллекции</Btn>
        <div style={{ flex: 1, minWidth: 200 }} />
        <Stepper currentId={step} onJump={setStep} />
      </div>

      {/* State switcher (proto-only) */}
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, padding: 10, background: "var(--wash)", border: "1px solid var(--line-2)", borderRadius: 10, flexWrap: "wrap" }}>
        <span className="eyebrow" style={{ marginRight: 4 }}>Состояние</span>
        <div className="tweaks__seg" style={{ flexWrap: "wrap" }}>
          {STEPS.map(s => (
            <button key={s.id} className={step === s.id && !saving && !savedOk ? "active" : ""} onClick={() => { setStep(s.id); setSaving(false); setSavedOk(false); }}>{s.label}</button>
          ))}
          <button className={saving ? "active" : ""} onClick={() => { setStep("review"); setSavedOk(false); setSaving(true); }}>Сохраняем…</button>
          <button className={savedOk ? "active" : ""} onClick={() => { setStep("review"); setSaving(false); setSavedOk(true); }}>Сохранён</button>
        </div>
      </div>

      {/* Two-column layout: form left, live map right */}
      <div className="planner-2col" style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 420px", gap: 22 }}>
...
// ======================================================================
// LIVE MAP COMPONENT — small SVG showing home / cities / return
// ======================================================================
function PlannerMap({ home, cities, returnCity, highlight }) {
  // Compute SVG points from lat/lng for all known cities involved
  const all = [];
  ...
  if (returnCity?.lat) {
    // Skip return if it equals home
    if (returnCity.city !== home.city) all.push({ key: "ret", name: returnCity.city, kind: "return", lat: returnCity.lat, lng: returnCity.lng });
  }

  // Map projection — naive linear in [-15, 60] lng / [25, 65] lat → [4..96, 8..72]
  const proj = (lat, lng) => {
    const lngMin = -15, lngMax = 60;
    const latMin = 25, latMax = 65;
  ...
  // Construct route line: home → cities → return
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span className="muted" style={{ fontSize: 11 }}>{points.filter(p => p.kind === "city").length} {points.filter(p => p.kind === "city").length < 5 ? "города" : "городов"}</span>
      </div>

      {/* Map canvas */}
      <div style={{ background: "#dceaf5", height: 320, position: "relative" }}>
        <svg viewBox="0 0 100 80" preserveAspectRatio="none" style={{ width: "100%", height: "100%", display: "block" }}>
          <defs>
            <pattern id="dots-planner" width="2" height="2" patternUnits="userSpaceOnUse">
              <circle cx="1" cy="1" r=".15" fill="#bcd4e8" />
            </pattern>
          </defs>
          <rect width="100" height="80" fill="#dceaf5" />
          <rect width="100" height="80" fill="url(#dots-planner)" />
          {/* Stylized European landmass */}
          <path
          ...
          {points.map(p => {
            ...
            const isActive =
              ...
              highlight === "all";
            const color = isHome ? "var(--brand)" : isReturn ? "var(--warm)" : "var(--brand)";
            return (
              <g key={p.key}>
                {isActive && <circle cx={p.x} cy={p.y} r="3.5" fill={color} opacity=".25" />}
                <circle cx={p.x} cy={p.y} r={isHome || isReturn ? 2 : 1.8} fill={color} />
                {(isHome || isReturn) && (
                  <text x={p.x + 3} y={p.y + 1.2} fontSize="2.4" fontWeight="700" fill="var(--ink)">{isHome ? "🏠 " : "↩ "}{p.name}</text>
                )}
                {p.kind === "city" && (
                  <>
      ...
      <div style={{
        ...
        <span style={{ flex: 1 }} />
        <span className="num">{cities.reduce((n, c) => n + (+c.nights || 0), 0)} ночей</span>
      </div>
    </div>
  );
}
...
// ======================================================================
// STEP 1: HOME (with "Рядом" + geolocation states)
// ======================================================================
function StepHome({ home, setHome, goNext }) {
  const [query, setQuery] = useState(home.city);
  // Geolocation state — "ask" / "allowed" / "denied"
  const [geoState, setGeoState] = useState("ask");
    ...
    <div>
      ...
      <Field label="Город старта">
        <div style={{ position: "relative" }}>
          ...
          <input className="input" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Москва, Тбилиси, Стамбул…" style={{ paddingLeft: 36, fontSize: 15 }} autoFocus />
        </div>
      </Field>

      {/* "Рядом" section with geolocation states — replaces old "Популярные" */}
      <div style={{ marginTop: 22, display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        ...
        {/* Proto-only state switcher */}
        <div className="tweaks__seg" style={{ background: "var(--wash)", fontSize: 11 }}>
          <button className={geoState === "ask" ? "active" : ""} onClick={() => setGeoState("ask")}>Не запрошена</button>
          <button className={geoState === "allowed" ? "active" : ""} onClick={() => setGeoState("allowed")}>Разрешена</button>
          <button className={geoState === "denied" ? "active" : ""} onClick={() => setGeoState("denied")}>Отклонена</button>
        </div>
      </div>
      ...
      {geoState === "ask" && (
        <div style={{
          ...
          <Btn variant="primary" size="sm" icon="pin" onClick={() => setGeoState("allowed")}>Разрешить</Btn>
        </div>
      )}
      ...
      {geoState === "allowed" && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 8 }}>
          {NEARBY_HOME.map((p) => (
            <button key={p.city} onClick={() => { setHome(p); setQuery(p.city); }} style={{
              display: "flex", alignItems: "center", gap: 10,
              padding: "12px 14px",
            ...
            onMouseLeave={(e) => { if (home.city !== p.city) e.currentTarget.style.borderColor = "var(--line)"; }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div className="muted" style={{ fontSize: 11.5 }}>{p.distance}</div>
              </div>
              {home.city === p.city && (
...
// ======================================================================
// STEP 2: CITIES (with drag-drop + date recalc)
// ======================================================================
function StepCities({ cities, setCities, home, goPrev, goNext }) {
  ...
  // Recompute startDates so each city follows the previous (firstStart + accumulated nights)
  const recomputeDates = (list) => {
  ...
  const addCity = () => {
    setCities(cs => recomputeDates([...cs, { id: Date.now(), city: "", country: "", startDate: cs[0]?.startDate || "", nights: 3 }]));
  };
  ...
  const update = (id, patch) => setCities(cs => {
    ...
    // If nights or first startDate changes, recompute downstream dates
  ...
  const onDrop = (id) => (e) => {
    ...
    setCities(cs => {
      ...
      // Dates recompute after reorder so each city follows the previous
  ...
  return (
    <div>
      ...
      <div className="muted" style={{ fontSize: 15, marginBottom: 22, maxWidth: 620 }}>
        Перечисли города в порядке поездки. <b style={{ color: "var(--ink-2)" }}>Перетащи</b> карточку за ручку слева — даты пересчитаются автоматически.
      </div>

      <CityAnchorRow label="Старт" city={home.city} country={home.country} kind="home" />

      {cities.length === 0 ? (
        <div style={{
          marginTop: 12, padding: 28,
          border: "1.5px dashed var(--line)",
          borderRadius: 12, textAlign: "center", color: "var(--muted)",
        }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 12 }}>
          {cities.map((c, i) => (
            <CityRow
              ...
              onMoveUp={() => setCities(cs => {
                if (i === 0) return cs;
                const ns = [...cs];
                [ns[i - 1], ns[i]] = [ns[i], ns[i - 1]];
                return recomputeDates(ns);
              })}
              onMoveDown={() => setCities(cs => {
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
        {POPULAR_DEST.filter(p => !cities.find(c => c.city === p.city)).slice(0, 6).map(p => (
          <button key={p.city} onClick={() => setCities(cs => recomputeDates([...cs, { id: Date.now(), ...p, startDate: cs[0]?.startDate || "" }]))} style={{
      ...
      {hasError && (
        <div style={{ marginTop: 16 }}>
          <Severity level="warning" title="Не хватает дат">
            Заполни даты заезда у всех городов, или их можно ставить позже из таймлайна.
          </Severity>
        </div>
      )}
      ...
      {cities.length > 0 && (
        <div style={{
          ...
          border: "1px solid var(--brand-soft-12)",
          borderRadius: 12,
      ...
      <FooterNav>
        ...
        <div style={{ flex: 1 }} />
        <Btn variant="primary" icon="arrowR" onClick={() => {
          if (cities.length === 0) { setHasError(true); return; }
          goNext();
        }}>Дальше</Btn>
      </FooterNav>
    </div>
...
function CityAnchorRow({ label, city, country, kind }) {
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: kind === "home" ? "var(--brand)" : "var(--ink-2)",
        color: "white",
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 13.5, fontWeight: 600 }}>{city} <span className="muted" style={{ fontWeight: 500, marginLeft: 6 }}>{country}</span></div>
      </div>
    </div>
  );
}
    ...
    <div
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
        <input
          ...
          value={city.nights}
          onChange={(e) => onChange({ nights: +e.target.value || 1 })}
      ...
      <div style={{ display: "flex", gap: 2 }}>
        <button onClick={onMoveUp} disabled={idx === 0} title="Выше" style={{
        ...
        <button onClick={onMoveDown} disabled={idx === total - 1} title="Ниже" style={{
          width: 26, height: 26, borderRadius: 6, border: "none",
  ...
  return (
    <div>
      <h1 style={{ marginBottom: 10, letterSpacing: "-0.025em" }}>
        Куда возвращаетесь после <span style={{ color: "var(--brand)" }}>{lastCity}</span>?
      </h1>
      <div className="muted" style={{ fontSize: 15, marginBottom: 22, maxWidth: 540 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
        <button onClick={() => setReturnMode("home")} style={{
          padding: 16, textAlign: "left",
          background: returnMode === "home" ? "var(--brand-soft)" : "var(--surface)",
          border: "1.5px solid " + (returnMode === "home" ? "var(--brand)" : "var(--line)"),
          ...
          <div className="muted" style={{ fontSize: 12.5, lineHeight: 1.4 }}>
            Обычный возврат. Triplanio добавит обратный переезд от <b>{lastCity}</b> в трип.
          </div>
        </button>

        <button onClick={() => setReturnMode("other")} style={{
          padding: 16, textAlign: "left",
          background: returnMode === "other" ? "var(--brand-soft)" : "var(--surface)",
          border: "1.5px solid " + (returnMode === "other" ? "var(--brand)" : "var(--line)"),
          borderRadius: 12, cursor: "pointer",
          display: "flex", flexDirection: "column", gap: 10,
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div className="muted" style={{ fontSize: 12.5, lineHeight: 1.4 }}>
            Например, если едешь дальше или летишь в командировку — укажешь куда.
          </div>
        </button>
      </div>

      {returnMode === "other" && (
        <Field label="Город возврата">
          <div style={{ position: "relative" }}>
            <Icon name="pin" size={15} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "var(--muted-2)" }} />
            <input className="input" placeholder="Куда летишь после трипа?" value={returnCity.city} onChange={(e) => {
  ...
  if (savedOk) {
    return (
      <div style={{ textAlign: "center", padding: "40px 20px" }}>
        ...
        <div className="muted" style={{ fontSize: 15, marginBottom: 22, maxWidth: 460, margin: "0 auto 22px" }}>
          «{tripTitle}» — {cities.length} {cities.length < 5 ? "города" : "городов"}, {totalNights} ночей. Можно добавлять отели, переезды и активности.
        </div>
        <div style={{ display: "inline-flex", gap: 8 }}>
          <Btn variant="primary" icon="arrowR" onClick={() => window.__navigate?.("timeline")}>Открыть таймлайн</Btn>
          <Btn variant="ghost" icon="map" onClick={() => window.__navigate?.("map")}>На карте</Btn>
        </div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ padding: 18 }}>
          <div className="eyebrow" style={{ marginBottom: 10 }}>Маршрут · {allCities.length} точек</div>
          ...
          <div style={{
            ...
            <Stat label="Стартовый бюджет" value="—" hint="Можно указать позже" />
          </div>
        </div>
      </div>
      ...
      {saving && (
        <div style={{
          ...
          <span className="ai-dots" style={{ color: "var(--brand)" }}>
            <span /><span /><span />
          </span>
          <div style={{ flex: 1, fontSize: 13, color: "var(--ink-2)" }}>
            Сохраняем трип — секунду…
          </div>
        </div>
      )}
      ...
      <FooterNav>
        ...
        {saving ? (
          ...
          <Btn variant="primary" icon="check" onClick={onSave}>Сохранить трип</Btn>
        )}
      </FooterNav>
    </div>
  );
...
function ReviewRow({ num, name, sub, icon, iconColor, muted }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "10px 0", position: "relative", zIndex: 1 }}>
      <div style={{
        width: 28, height: 28, borderRadius: "50%",
        background: icon ? iconColor : "var(--brand)",
        color: "white",
        display: "grid", placeItems: "center",
        fontSize: 11, fontWeight: 700, flexShrink: 0,
...
export default ScreenManualPlanner;

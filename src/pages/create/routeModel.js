...
/**
 * ★ ГДЕ ФИНИШ — ОДИН ОТВЕТ НА ВЕСЬ ФЛОУ (TRIP-484).
 *
 * Финиш выражается ТРЕМЯ разными формами узлов, и до этой функции каждая
 * поверхность выводила его сама: шаг городов считал финишем только отдельный
 * узел, шаг возврата — ещё и помеченный город, пропуск шага — третьим способом.
 * Поверхности расходились МОЛЧА: «останусь в X», выбранное на шаге возврата,
 * шаг городов рисовал обычным городом, шаг возврата при этом исчезал (финиш
 * «уже есть»), и поменять решение становилось нечем.
 *
 * Формы:
 *   city — отдельный узел `end` (в том числе «домой»: клон старта);
 *   stay — ПОСЛЕДНИЙ ГОРОД помечен финишем, ночи при нём (в списке он город);
 *   home — узла нет, но есть старт: молчаливый дефолт «вернусь домой», он же
 *          дописывается в нагрузку (`toCitiesPayload`);
 *   none — узла нет и старта нет: сказать нечего.
 *
 * `decided` отвечает на вопрос «человек ВЫБРАЛ финиш», а не «финиш известен»:
 * дефолт «домой» выбором не является, иначе шаг возврата пропускался бы у всех.
 *
 * @param {any[]} nodes
 * @returns {{ mode: 'city'|'stay'|'home'|'none', node: any, decided: boolean }}
 */
export function finishOf(nodes) {
  const end = endOf(nodes);
  if (end) return { mode: isStayNode(end) ? 'stay' : 'city', node: end, decided: true };
  const start = startOf(nodes);
  return start ? { mode: 'home', node: start, decided: false } : { mode: 'none', node: null, decided: false };
}

/** Человек выбрал финиш (обе формы выбора). Дефолт «домой» — не выбор. */
export const hasExplicitEnd = (nodes) => finishOf(nodes).decided;

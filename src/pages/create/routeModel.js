 ...
 * Карта — для списка (редактор строит её один раз на отрисовку), `visitNumberOf`
 * — для одиночного ряда; второго ВЫРАЖЕНИЯ правила нет, только два доступа.
...
/** Номер ОДНОГО города (1-based); 0 = номера нет (якорь, пересадка, чужой узел). */
export const visitNumberOf = (nodes, node) => visitNumbers(nodes)[String(node?.id)] || 0;
 ...
 *
 * ⚠️ ЯКОРЬ ЕДЕТ БЕЗ НОЧЕЙ. Ночей у него и в списке нет: `makeNode` их якорю не
 * выдаёт, `withNights` якоря не трогает. Проекция это лишь повторяет — второго
 * правила «у финиша ночей не бывает» здесь не заводится.
 *
 * ⚠️ ФИНИША ПО УМОЛЧАНИЮ НЕТ. Нет узла `end` — маршрут кончается последним
 * городом, и наружу едет ровно это. Прежний вывод `finishCity = end || home`
 * («не выбрал» молча значило «домой») снят целиком: возврат домой существует
 * только как ЯВНО выбранный узел.
 * @param {RouteNode[]} nodes
 */
export function toCitiesPayload(nodes) {
  ...
  // Клона старта здесь нет намеренно (разбор в докстринге): «домой» создаёт
  // карточка на шаге возврата — тем же узлом `end`, что и любой другой финиш.
  return out;
}

...
export default function FlowMap({
  ...
  // ★ СВОБОДНОЕ ОКНО В ПОДПИСЬ КАДРА НЕ ВХОДИТ, И ЭТО РЕШЕНИЕ, А НЕ УПУЩЕНИЕ.
  // Автофокус случается ТОЛЬКО на изменение МАРШРУТА (добавили/удалили город,
  // переставили порядок). Смена детента шита — не новая цель кадра, а та же
  // самая, показанная в окне другого размера: карта под него подстраивается
  // СДВИГОМ холста (центр маршрута остаётся в центре свободного окна), но зум и
  // границы не трогает никто. Положи окно сюда — и каждая осадка шита вписывала
  // бы маршрут заново, то есть «карта сама наводится», чего быть не должно.
  //
  // Цена решения названа прямо: подняв шит после кадрирования при опущенном,
  // пользователь закроет им часть маршрута — окно стало меньше, а кадр остался
  // прежним. Это его собственный жест, и восстанавливается он тем же жестом.
  // Размер ВЬЮПОРТА (`winW`/`winH`) в подписи остаётся: поворот экрана и
  // переход десктоп↔телефон — смена раскладки, а не жест над картой.

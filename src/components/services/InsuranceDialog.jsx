/**
 * InsuranceDialog — unified view + edit dialog for kind="insurance" trip services.
 *
 * View mode:  Lumo-styled header (insurance color), sections for policy /
 *             cost / notes / docs.
 * Edit mode:  Form — name, price, currency, policy_number, date_start,
 *             date_finish, notes, documents[].
 *             Extra fields stored in details JSONB.
 *
...
import { Loader2, ShieldCheck, Trash2, Edit2, X, FileText, ExternalLink } from 'lucide-react';
...
// ─── Lumo tokens for Insurance ───────────────────────────────────────────────
const INS_COLOR = 'var(--ev-insurance)';
const INS_INK   = 'var(--ev-insurance-ink)';
const INS_SOFT  = 'var(--ev-insurance-soft)';

// ─── Primitives ──────────────────────────────────────────────────────────────

function SectionLabel({ color, children }) {
  const c = color || INS_COLOR;
  return (
    <div style={{
      display: 'flex', alignItems: 'center', gap: 7,
      fontSize: 10, fontWeight: 800, textTransform: 'uppercase',
      letterSpacing: '.06em', color: c, marginBottom: 6,
    }}>
      <span style={{ width: 3, height: 12, borderRadius: 2, background: c, flexShrink: 0 }} />
      {children}
      <span style={{ flex: 1, height: 1, background: `color-mix(in srgb, ${c} 18%, transparent)` }} />
    </div>
  );
}
...
function InsuranceView({ service, t, onEdit, onDelete, onClose, canEdit, deleting, confirmDel, setConfirmDel }) {
  ...
  return (
    <>
      ...
      <div style={{
        ...
        padding: '18px 20px 16px', background: INS_SOFT,
        borderBottom: '1px solid var(--line-2)',
      }}>
        <div style={{
          width: 44, height: 44, borderRadius: 14, background: INS_COLOR,
          color: '#fff', display: 'grid', placeItems: 'center', flexShrink: 0,
          boxShadow: `0 4px 12px -4px color-mix(in srgb, ${INS_COLOR} 50%, transparent)`,
        }}>
          <ShieldCheck style={{ width: 21, height: 21 }} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 10, fontWeight: 800, color: INS_INK, textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 3 }}>
            {t('service.insurance_eyebrow')}
          </div>
      ...
      <div style={{ padding: '15px 20px', flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 11 }}>
        {confirmDel ? (
          <div style={{ borderRadius: 12, border: '1px solid color-mix(in srgb, var(--danger) 30%, transparent)', background: 'var(--danger-soft)', padding: 16, display: 'flex', alignItems: 'flex-start', gap: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 600, fontSize: 14 }}>{t('event.delete_q', { label: t('service.kind.insurance').toLowerCase() })}</div>
          ...
          <>
            {/* Policy section */}
            <SecCard>
              <SectionLabel>{t('service.insurance_section')}</SectionLabel>
              <KVGrid>
                <KV label={t('service.name')} full>{service.name}</KV>
            ...
            {/* Cost */}
            <SecCard>
              <SectionLabel>{t('service.insurance_cost_section')}</SectionLabel>
              <KVGrid>
            ...
            {docs.length > 0 && (
              <SecCard>
                <SectionLabel>{t('service.insurance_docs_section')} · {docs.length}</SectionLabel>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {docs.map((doc, i) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 10, border: '1px solid var(--line)', background: 'var(--surface)', textDecoration: 'none', color: 'var(--ink-2)', fontSize: 13, fontWeight: 700 }}
                    >
      ...
      <div style={{ padding: '11px 20px', borderTop: '1px solid var(--line-2)', background: 'var(--wash)', display: 'flex', gap: 7, alignItems: 'center' }}>
        {confirmDel ? (
          ...
          <>
            ...
            {canEdit && (
              <Btn size="sm" onClick={onEdit} style={{ background: INS_COLOR, borderColor: INS_COLOR, color: '#fff' }}>
...
function InsuranceEdit({ service, tripId, t, onClose, canEdit }) {
  const { user } = useAuth();
  ...
  const [dateStart, setDateStart]       = useState(d.date_start || '');
  const [dateFinish, setDateFinish]     = useState(d.date_finish || '');
  const [notes, setNotes]               = useState(d.notes || '');
  const [docs, setDocs]                 = useState(() => Array.isArray(d.documents) ? d.documents : []);
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <div style={{ width: 44, height: 44, borderRadius: 14, background: INS_COLOR, color: '#fff', display: 'grid', placeItems: 'center', flexShrink: 0, boxShadow: `0 4px 12px -4px color-mix(in srgb, ${INS_COLOR} 50%, transparent)` }}>
          <ShieldCheck style={{ width: 21, height: 21 }} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 10, fontWeight: 800, color: INS_INK, textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 3 }}>
          ...
          <h2 style={{ margin: 0, fontSize: 17, fontWeight: 600, color: 'var(--ink)', lineHeight: 1.2 }}>
            {isEdit ? t('service.insurance_edit') : t('service.insurance_new')}
          </h2>
        </div>
        <button onClick={onClose} style={{ width: 30, height: 30, borderRadius: 9, border: '1px solid var(--line-strong)', background: 'var(--surface)', color: 'var(--muted)', display: 'grid', placeItems: 'center', cursor: 'pointer', flexShrink: 0 }}>
          <X style={{ width: 15, height: 15 }} />
      ...
      <div style={{ padding: '15px 20px', flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <Field label={`${t('service.name')} *`}>
          <input className="input" value={name} onChange={(e) => setName(e.target.value)}
            placeholder="AXA Travel, Allianz…" autoFocus />
        </Field>
        ...
        <Field label={t('service.policy_number')}>
          <input className="input" value={policyNumber} onChange={(e) => setPolicyNumber(e.target.value)}
            placeholder={t('service.policy_number_ph')} style={{ fontFamily: 'var(--font-mono)' }} />
        </Field>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9 }}>
          <Field label={t('service.date_start')}>
            <input className="input" type="date" value={dateStart} onChange={(e) => setDateStart(e.target.value)} />
          </Field>
          <Field label={t('service.date_finish')}>
            <input className="input" type="date" value={dateFinish} onChange={(e) => setDateFinish(e.target.value)} />
          </Field>
        </div>

        <Field label={t('service.notes')}>
          <textarea className="textarea" value={notes} onChange={(e) => setNotes(e.target.value)}
            placeholder={t('service.insurance_notes_ph')} style={{ minHeight: 72 }} />
        </Field>

        {/* Docs */}
        <div>
          <div style={{ fontSize: 10.5, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.06em', color: INS_COLOR, display: 'flex', alignItems: 'center', gap: 7, margin: '4px 0 8px' }}>
            {t('service.insurance_docs_section')}
            <span style={{ flex: 1, height: 1, background: `color-mix(in srgb, ${INS_COLOR} 18%, transparent)` }} />
          </div>
          <DocumentsField
            value={docs}
            onChange={setDocs}
      ...
      <div style={{ padding: '11px 20px', borderTop: '1px solid var(--line-2)', background: 'var(--wash)', display: 'flex', gap: 7, alignItems: 'center' }}>
        ...
        {!confirmDel && (
          <>
            ...
            <Btn
              size="sm"
              style={{ background: INS_COLOR, borderColor: INS_COLOR, color: '#fff' }}
...
export default function InsuranceDialog({ open, onOpenChange, tripId, service, canEdit = false, defaultEditMode = false }) {

...
export default function CityAdder({ onAdd, hasStart, hasEnd, defaultKind = 'transit', renderTrigger, onOpenChange }) {
  ...
  /* ★ ОТКРЫТОСТЬ СООБЩАЕТСЯ НАРУЖУ ОДНИМ КАНАЛОМ. Читатель у неё один: шаг
     визарда не даёт нажать «Далее», пока работа с городом не закончена
     (сохранена или отменена) — иначе уход со шага бросает наполовину введённый
     город, а он нигде не сохранён. Второго способа узнать этот факт нет.
     ⚠️ Уступка места пустому состоянию ЧЕРЕЗ КАНАЛ НЕ ИДЁТ и читателем его не
     является: на десктопе композер рисуется ВМЕСТО своего триггера
     (`if (!open) return trigger`), то есть это его собственное устройство.

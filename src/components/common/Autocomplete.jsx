 ...
 * (`interactive-widget=resizes-content`), якорь уезжает - и Radix честно
    ...
    return (
      <>
        {/* Триггер — ТО ЖЕ поле, а не его двойник: те же декорации (флаг
            выбранного города, «×», состояние валидации из `inputProps`), тот же
            скин. И это НАСТОЯЩЕЕ поле, а не кнопка в его обличье, — потому что
            клавиатуру поднимает тап по полю, а не наш код.
            ★ ЗДЕСЬ НЕТ НИ `readOnly`, НИ `preventDefault`, И ЭТО НЕСУЩЕЕ. Обе
            эти вещи тут были, и обе ГАСЯТ КЛАВИАТУРУ на iOS: `readOnly` — прямо
            (в такое поле не печатают), `preventDefault` на mousedown — косвенно
            (Safari читает его как отказ приложения от штатного фокуса и снимает
            с последующего программного `focus()` право поднять клавиатуру).
            Так шторка и открывалась без клавиатуры при исправной геометрии.
            Теперь тап фокусирует поле НАТИВНО — клавиатуру поднимает платформа,
            — а `onFocus` тут же распахивает шторку, и та перенимает фокус на своё
            поле. Переход между текстовыми полями клавиатуру не роняет.
            Открываем на `focus`, а не на `click`: он раньше, и между поднятием
            клавиатуры и появлением шторки не остаётся кадра, в котором страница
            успела бы дёрнуться к полю. Обратно фокус не вернётся — шторка это
            запрещает (`onCloseAutoFocus`), поэтому петли «вернули фокус ->
            открылись снова» нет по построению. Кнопка «×» гасит своё событие
            сама и фокуса не даёт. */}
        <Input
          icon={icon}
          value={inputValue || ''}
          onChange={handleChange}
          onFocus={() => { if (!disabled) openSheet(); }}

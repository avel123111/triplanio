...
function RailItem({ icon, label, active = false, badge = 0, pro = false, onClick }) {
  return (
    <button
      ...
      // («Planificación», «Presupuesto») в узкой колонке режутся, и полное имя обязано
...
// Левое меню трипа — ПЛАВАЮЩИЙ ВИДЖЕТ под шапкой (90 px, зазор со всех сторон).
// Рисуется ОДИН раз оболочкой TripShell, все секции переключаются одним и тем же
// onNavigate.
//
// Бренд-слота у меню НЕТ: знак живёт в шапке, а шапка идёт во всю ширину экрана.
// Выход из трипа — круглая кнопка в ней же, на всех ширинах; прежде он прятался
// в знак и проступал по наведению, потому что круглой кнопке на левом борту
// места не было.
//
...
// которых в узкой колонке места нет. Общий у них ровно источник пунктов.
    ...
    <aside className="app-side">
      <nav className="app-side__nav">
        ...
        {(mgmtRows.length > 0 || showTail) && (
          /* Подпись группы в узкой колонке не живёт — её работу делает черта, которую

 ...
 * экранах ВНЕ трипа (у самой шапки поля слева там нет). Внутри трипа поле даёт
 * сама шапка (`.app-header--trip`), и слот там сжат до знака — иначе поле
 * сложилось бы с его шириной.
 *
 ...
 *
 * Подписи и подсказки у слота нет, и это не упущение: имя кнопке даёт `alt`
 * логотипа, а рядом в шапке стоит само слово. Проп `title` (он же подсказка и
 * `aria-label`) обслуживал ВТОРОЙ дом — слот в рейле, где знак работал выходом
 * из трипа и без подписи был бы непонятен. Дома не стало — не стало и пропа;
 * компонент перестал быть экспортом по той же причине.
 *
 * ★ Вместе с пропом ушла и обёртка `<Tooltip>` — узел ДС, поэтому доля ДС падает
 * на 2. Это не деградация языка: пузырь показывал ровно `title`, а его больше
 * нет, и оставить обёртку значило бы держать примитив ради пустого содержимого.
 * floor-exempt: dsshare +2 — снята подсказка вместе с пропом `title`, которому она служила (второй дом слота закрыт), апрув Pavel
 *
 * @param {{ onClick: () => void }} p
 */
function BrandSlot({ onClick }) {
  return (
    <button className="app-header__brand" onClick={onClick} type="button">

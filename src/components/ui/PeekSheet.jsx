...
/**
 * Насколько визуальный вьюпорт СМЕЩЁН вниз относительно раскладочного.
 * Вьюпорт объявлен `interactive-widget=resizes-content`: на Chrome раскладка
 * сжимается вместе с видимой областью, и смещение там ноль. Там, где браузер
 * вместо сжатия СДВИГАЕТ окно (iOS-движок, директиву не понимающий),
 * `position: fixed` остаётся привязан к раскладке, и без этого слагаемого низ
 * шита оказался бы под клавиатурой.
 * ⚠️ Замер на устройстве: `offsetTop` держался нулём весь сеанс — то есть
 * слагаемое это защита ценой в ноль, а не измеренная необходимость, и тестом
 * оно намеренно не требуется (высота — требуется).
 */

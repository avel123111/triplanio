...
/**
 * ★★ ВЫСОТА ВЬЮПОРТА — РАСКЛАДОЧНАЯ, И КЛАВИАТУРА ЕЁ НЕ КАСАЕТСЯ (TRIP-535).
 *
 * Здесь стоял `visualViewport.height` — «то, что видно», — и это был корень
 * дрожи, переживший три захода. Клавиатура сжимает видимую область ПАЧКОЙ
 * промежуточных значений (движок не обещает ни их числа, ни темпа), а от этой
 * величины считаются ВСЕ детенты и положение шита. То есть на каждый чих
 * движка пересчитывалась вся геометрия, и `transform` вёз шит к цели своим
 * транзишном, пока цель уже уехала. Лечить это признаками «клавиатура моя»,
 * порогами и таймерами тишины бессмысленно: каждый такой признак — ещё один
 * вход, а входы приходят в непредсказуемом порядке, и снаружи это выглядит как
 * «дёргается случайным образом».
 *
 * Раскладочный вьюпорт под клавиатуру не ужимается ни на одном движке, который
 * нас касается (на iOS — вообще никогда, замер в шапке `keyboardOpen.js`), и
 * меняется только от поворота и смены окна. Значит геометрия шита от клавиатуры
 * НЕ ЗАВИСИТ ВООБЩЕ — пересчитывать нечего, дрожать нечему.
 *
 * Место под клавиатуру даёт CSS вычетом `--kb-h` у ТЕЛА (не у футера: его высота
 * измеряется и вернулась бы в детенты петлёй), а не арифметика здесь.
 */
function viewportH() {
...
export function PeekSheet({
  ...
  // ★★ ЕДИНСТВЕННАЯ РЕАКЦИЯ НА КЛАВИАТУРУ — ОДИН БУЛЕВ ПРИЗНАК (TRIP-535).
  //
  ...
  // клавиатуру открыли, оказывается за кадром.
  //
  // ⚠️ «СВОЯ» — НЕСУЩЕЕ СЛОВО. Поле ввода живёт и в шторке, которая ложится
  // ПОВЕРХ шита (пикер города), и глобальный флаг клавиатуры там врёт: поднимает
  // её ЧУЖАЯ поверхность, а прыгает этот шит (TRIP-484 §4). Владение решает
  // фокус: клавиатуру физически поднимает сфокусированное ТЕКСТОВОЕ поле, а
  // портал шторки живёт в `document.body`, поэтому `contains` даёт `false`.
  //
  // ★★ ПОЧЕМУ ЗДЕСЬ БОЛЬШЕ НЕТ НИ ГЕОМЕТРИИ, НИ ТАЙМЕРОВ, НИ ПОРОГОВ. Три
  // предыдущие редакции по очереди добавляли вход: просадку видимой области на
  // 120 px, окно вокруг движения, тишину во `visualViewport`, признак «она
  // поднималась». Каждый вход в отдельности был обоснован, а вместе они —
  // автомат на пяти сигналах, которые движок присылает в НЕ ОПРЕДЕЛЁННОМ
  // порядке и количестве. Снаружи это ровно «дёргается случайно»: поведение
  // зависело от того, чьё событие успело первым. Ни одно из этих правил не
  // вернулось: признак — ЧИСТАЯ ФУНКЦИЯ ФОКУСА, у него два значения и один
  // источник, и порядок событий на него не влияет никак.
  //
  // Геометрию клавиатура не трогает вовсе (`viewportH` выше), место под неё даёт
  // CSS-отступ у коробки. Поэтому переход ровно один — `restY` ⇄ 0 — и обе точки
  // от клавиатуры не зависят: транзишн доезжает до неподвижной цели.
  //
  // ⚠️ ГЕЙТ УСТРОЙСТВА: фокус в поле бывает и там, где экранной клавиатуры нет,
  // и без гейта клик в textarea на десктопе уводил бы шит на верхний детент.
  const [keyboardMine, setKeyboardMine] = useState(false);
  useEffect(() => {
    if (!hasSoftKeyboard()) return undefined;
    // `focusin`/`focusout` всплывают, поэтому хватает двух подписок на документ.
    // `focusout` приходит РАНЬШЕ, чем встал новый фокус (`activeElement` в этот
    // момент — `body`), поэтому пересчёт отложен на микрозадачу: к ней фокус уже
    // на месте, и переезд «поле шторки → поле шита» не даёт ложного закрытия.
    const sync = () => queueMicrotask(() => setKeyboardMine(isMine(sheetRef.current)));
  ...
  const measure = useCallback(() => {
    ...
    // Высота — раскладочная, клавиатура её не двигает (разбор у `viewportH`),
    // поэтому спрашивать «чья клавиатура» тут больше нечего: пересчёт безопасен
    // всегда. Читаем DOM, а не состояние: `measure` навешан один раз и
    // реактивных значений не видит.
    setVh(viewportH());
  }, []);
  ...
  useLayoutEffect(() => {
    ...
    // ★ ТОЛЬКО `window.resize` — поворот и смена окна. На `visualViewport` шит
    // БОЛЬШЕ НЕ ПОДПИСАН намеренно: это был единственный канал, по которому
    // клавиатура двигала геометрию, и именно он пересчитывал всё на каждое из
    // непредсказуемо многих событий её выезда.
  ...
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      // ★ ТЯГА СНИМАЕТ ФОКУС. Пока поле в фокусе, шит держится на верхнем
      // детенте (это и есть вся реакция на клавиатуру), поэтому без блюра он
      // отскакивал бы обратно на отпускании — «шит не опускается». Тянуть шит и
      // означает «я дописал»: так же ведёт себя любой системный шит с полем.
      // Вдобавок это закрывает случай, когда клавиатуру гасят БЕЗ blur
      // (системная «назад» на Android): у человека всегда есть жест, которым
      // состояние разбирается.
      if (d.mode === 'drag' && isMine(el)) document.activeElement?.blur?.();

 ...
 *
 * ⚠️ У ЭТОЙ ПАРЫ ЕСТЬ ИЗВЕСТНАЯ ЦЕНА, И ОНА НЕ ЗДЕСЬ. `resizes-content` означает
 * «клавиатура пересчитывает вьюпорт РАСКЛАДКИ», то есть страница под шитом
 * перекладывается — на Chrome/Android заметно, на iOS-движке нет (WebKit эту
 * директиву не поддерживает и раскладку не трогает). Замер на стенде: при
 * усадке видимого до 480 вся страница ехала 0..844 → 0..480 и возвращалась.
 * Обратная пара (`overlays-content` + подъём средствами vaul) это снимает, но
 * трогает КАЖДЫЙ экран с полем ввода на обеих платформах, поэтому идёт отдельной
 * задачей со своей проверкой на устройстве, а не попутно.
 *

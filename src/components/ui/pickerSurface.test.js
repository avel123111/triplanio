...
test('★★ выбор строки в ОБОИХ движках висит на паре pointerdown/pointerup, а не на click', () => {
  /* ★ ЧТО ВИДЕЛ ЧЕЛОВЕК (iPhone/WebKit, замер). Набрал город подсказкой
     клавиатуры — список появился, строка от тапа подсветилась, а город не
     выбрался. Помогал только Enter. Подсветку ставит `onMouseEnter`, выбор
     ставил `onClick` — значит совместимая мышиная цепочка ДОХОДИЛА до строки, а
     `click` терялся: WebKit держит слово из подсказки как незакоммиченную
     композицию (marked text) и тратит первое касание вне поля на её коммит.
     Второй, независимый источник той же потери — переезд списка между down и
     up (отступ скроллера под клавиатуру, ответ нового поиска): `click` рождается
     на ОБЩЕМ ПРЕДКЕ двух целей, то есть на контейнере, а не на строке.
     Поэтому выбор снят с `click` у ОБОИХ движков сразу: строка листа у них
     физически одна (`.ss-opt`), поверхность одна (`PickerSheet`) и поле поиска
     одно — дефект общий, и лечение обязано быть общим, иначе завтра починят
     один экран из двух. Само решение «тап или скролл» — чистая функция
     `lib/tapGesture`, её гейт `tapGesture.test.js`. */
  for (const rel of ['components/common/Autocomplete.jsx', 'components/ui/SearchSelect.jsx']) {
    const code = codeOf(join(SRC, rel));
    assert.match(code, /from '@\/lib\/tapGesture'/,
      `${rel}: решение «тап или скролл» обязано приходить из общего модуля, а не быть переписано на месте`);
    assert.match(code, /onPointerDown=\{\(e\)\s*=>\s*\{\s*tapRef\.current\s*=/,
      `${rel}: нажатие обязано ЗАПОМИНАТЬ строку — на отпускании под пальцем может быть уже другая`);
    assert.match(code, /onPointerUp=/, `${rel}: выбор делает pointerup`);
    assert.match(code, /onPointerCancel=/,
      `${rel}: браузер забирает жест под скролл через pointercancel — без обработчика скролл списка выберет город`);
    assert.match(code, /tapPick\(tapRef\.current/,
      `${rel}: решение принимает общая функция, а не условие на месте`);
    assert.match(code, /if\s*\(pickedRef\.current\)\s*return;/,
      `${rel}: click остаётся входом для клавиатуры и ВТ — без защёлки один тап выберет дважды`);
    assert.match(code, /if\s*\(picked\s*!==\s*null\)/,
      `${rel}: «не выбрано» — это строго null: опцией листа законно бывает 0 или пустая строка, и по истинности такая не выберется НИКОГДА и молча`);
  }
});

...
test('поверхность НЕ ставит фокус — ни хуком, ни руками, ни по таймеру', () => {
  const surface = readFileSync(join(UI, 'PickerSheet.jsx'), 'utf8')
    .replace(/\/\*[\s\S]*?\*\//g, '').replace(/^\s*\/\/.*$/gm, '');

  // ★ ИНВАРИАНТ РАЗВЁРНУТ ПО ЗАМЕРУ НА УСТРОЙСТВЕ. Прежняя редакция требовала
  // ОБРАТНОГО — «фокус ставит штатный хук `onOpenAutoFocus`» — и была верна ровно
  // до того, как триггер стал кнопкой. С кнопкой платформа клавиатуру по
  // программному фокусу не даёт: каретка появляется, ввода нет, и вдобавок она
  // ставится посреди въездной анимации vaul, поэтому на WebKit видна ОТДЕЛЬНО от
  // поля и потом «догоняет» его.
  // Значит правильного момента для этого фокуса не существует, и лечится это не
  // ожиданием (`transitionend`/таймер — уже пробовали и выпилили), а тем, что
  // ставить нечего.
  assert.ok(!surface.includes('onOpenAutoFocus'),
    'программный фокус на открытии даёт каретку без клавиатуры — ставить его нельзя ни в каком виде');
  assert.ok(!/\.focus\(/.test(surface),
    'ручной focus() — та же каретка без клавиатуры, только в обход штатного хука');
  assert.ok(!/useEffect|useLayoutEffect|requestAnimationFrame|setTimeout|transitionend|animationend/.test(surface),
    'ожидание анимации лечит симптом: причина в том, что фокус тут не даёт клавиатуры вовсе');
});

...
test('дисциплина фокуса записана ОДИН раз, движки её зовут', () => {
  ...
  // Прежде здесь требовался `useLayoutEffect` — под вход БЕЗ триггера. Тот вход
  // удалён (он не мог дать клавиатуру: портал монтируется не в том же кадре), и
  // требование ушло вместе с ним, а не «ослабло».
  assert.ok(!/useLayoutEffect|useEffect/.test(bare),
    'у поверхности не осталось входа, которому нужен фокус вне жеста');
});

...
test('лист у пикера ОДИН — на обе поверхности', () => {
  // ★ ЧТО ИМЕННО ЛОВИТСЯ. Контейнер листа несёт `id`, на который ссылается
  // `aria-controls` поля, роль `listbox` и условия показа пустоты и атрибуции.
  // Написанный дважды (шторка и попап), он и разъезжается по УСЛОВИЯМ, а не по
  // виду: первая копия показывала «ничего не найдено», вторая — нет; первая
  // гейтила атрибуцию по `results.length`, вторая — нет. Ни один гард такого не
  // видит, потому что классы и там, и там одни и те же.
  // Поймано гардом 2o косвенно (сырые теги), а не глазами, — и это причина, по
  // которой инвариант пиннится здесь, а не остаётся уроком в шапке.
  const src = readFileSync(join(UI, '../common/Autocomplete.jsx'), 'utf8')
    .replace(/\/\*[\s\S]*?\*\//g, '').replace(/^\s*\/\/.*$/gm, '');
  assert.equal((src.match(/role="listbox"/g) || []).length, 1,
    'второй контейнер листа = развилка поведения между шторкой и попапом');
  assert.equal((src.match(/\{listEl\}/g) || []).length, 2,
    'обе поверхности обязаны рендерить ОДИН И ТОТ ЖЕ узел, а не свой каждая');
});

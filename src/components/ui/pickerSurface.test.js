...
test('коробка шторки не везёт на себе сфокусированное поле', () => {
  // ★ ЗАМЕР, А НЕ ВКУС (витрина /kit, живой компонент, покадрово):
  //   было  поле едет 931 -> 87 px за ~500 мс, всё это время В ФОКУСЕ
  //   стало поле стоит на 87 с нулевого кадра, размах 0 px; едет ЛИСТ (236 px)
  // Каретке нечего догонять, только если под ней ничего не двигается. Чем
  // двигали — трансформом или раскладкой — неважно: пробовали оба, прыгало оба
  // раза. Поэтому пиннится ИМЕННО неподвижность коробки.
  //
  // ★★ И ПИННИТСЯ СПЕЦИФИЧНОСТЬ, ПОТОМУ ЧТО ИМЕННО ОНА ОДИН РАЗ УЖЕ СЪЕЛА ФИКС.
  // Своё правило въезда vaul объявляет ЧЕТЫРЬМЯ атрибутами. Правило
  // класс+два атрибута (0,3,0) ему проигрывает и не применяется ВООБЩЕ — молча:
  // объявление в файле есть, браузер берёт чужое, снаружи «ничего не изменилось».
  // Ровно это и уехало на превью прошлым заходом.
  ...
  for (const attr of ['data-vaul-snap-points', 'data-vaul-drawer-direction']) {
    assert.ok(sel.includes(attr),
      `без ${attr} селектор проигрывает vaul по специфичности и не применяется — фикс станет невидимым`);
  }
  assert.ok(!/@keyframes\s+sheetListRise[^}]*}[^@]*top:/.test(css),
    'лист едет трансформом; въезд коробки по top — это прошлая, отменённая редакция');
  assert.match(css, /@keyframes sheetListRise/, 'движение обязано остаться: едет лист');

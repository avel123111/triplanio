...
test('коробка ПОЛНОЭКРАННОЙ поверхности НЕ зависит от клавиатуры, и правило ОДНО', () => {
  ...
  // Осталось от редакции, которую сами же и отменили: коробка от видимой полосы
  // пересчитывала процентный трансформ въезда. Тест был зелёный и охранял верное
  // свойство, но врал о том, какое.
  //
...
test('дисциплина фокуса записана ОДИН раз, движки её зовут', () => {
  ...
  for (const rel of ['SearchSelect.jsx', '../common/Autocomplete.jsx']) {
    ...
    for (const [needle, why] of [
      ...
      // Запрещённое здесь — трогать ЧУЖОЙ фокус; это пиннит отдельный тест
      // «чужой фокус не трогает НИКТО, кроме поверхности».
    ]) {
...
test('закрытие снимает фокус — у ПОВЕРХНОСТИ, а не у одной из четырёх дверей', () => {
  ...
  for (const rel of ['SearchSelect.jsx', '../common/Autocomplete.jsx']) {
    ...
    // «закрываюсь»). Нельзя другое: трогать ЧУЖОЙ фокус. Именно так был написан
    // снятый `blurOnPick` — `document.activeElement.blur()` через кадр, то есть
    // удар по узлу, которому Radix уже вернул фокус.
...
test('чужой фокус не трогает НИКТО, кроме поверхности', () => {
  ...
  // которому Radix уже вернул фокус. Нужда была настоящей (на шаге визарда
  // `:focus-within` прячет дату), но это свойство ПИКЕРА, а не экрана: оно
  // переехало в `Autocomplete.pick()` и накрыло все вызывателя и обе поверхности.
  // Единственный, кому позволено трогать чужой фокус, — поверхность, и только
  // внутри своего поддерева (`contains`).

...
test('панель едет, поле стоит — обе группы объявлены явно', () => {
  ...
  /* ★★ УСЛОВИЕ ВЪЕЗДА — ПРИШПИЛЕННОЕ ПОЛЕ, А НЕ ИМЯ ПОВЕРХНОСТИ (TRIP-494).
     Приём существует ровно затем, чтобы поле с кареткой не ехало, — значит и
     адресовать он обязан ТО, У ЧЕГО ПОЛЕ ЕСТЬ. Ключ по семье накрыл бы и панель
     редактора, у которой нет ни `.ss-list`, ни `.ss-search`: она получила бы
     ПОЛОВИНУ анимации (краска едет, содержимое стоит). */
  assert.ok(sel.includes(':has(.ss-search)'),
    'въезд обязан адресовать поверхность С ПРИШПИЛЕННЫМ ПОЛЕМ, иначе панель редактора получит половину анимации');
  ...
  /* ⚠️ УСЛОВИЕ ВЫРЕЗАЕТСЯ ПЕРЕД РАЗБОРОМ ЦЕЛЕЙ. `:has(.ss-search)` называет, У
     КОГО есть пришпиленное поле, а проверка ниже — про то, ЧТО едет. Без этого
     наивный `includes('.ss-search')` читает условие как цель и краснеет на
     верном коде: имя поля стоит в селекторе, но пассажиром оно не стало. */
  const block = ride.slice(0, ride.indexOf('}') + 1).replace(/:has\([^)]*\)/g, '');

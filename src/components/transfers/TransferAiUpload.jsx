...
const SEGMENT_SCHEMA = {
  ...
  properties: {
    transport_type: {
      type: 'string',
      enum: ['plane', 'train', 'bus', 'car', 'taxi', 'ferry', 'other'],
    },
    ...
    carrier: { type: 'string', description: 'Airline / train operator / bus company name (e.g. Lufthansa, Renfe, Aeroexpress)' },
    booking_reference: { type: 'string', description: 'PNR / record locator / booking number' },
    from_address: { type: 'string', description: 'Full departure address (airport, station, hotel)' },
    to_address: { type: 'string', description: 'Full arrival address' },
    price: { type: 'number' },
...
const EXTRACT_SCHEMA = {
  ...
  properties: {
    ...
    booking_platform: { type: 'string', description: 'e.g. aviasales, skyscanner, omio, trainline, kiwi, etc.' },
    segments: { type: 'array', items: SEGMENT_SCHEMA },
  },
};

// Inject the real current year so the LLM doesn't default to its training-data
// year (e.g. 2024) when the ticket omits the year.
const CURRENT_YEAR = new Date().getFullYear();
const TODAY_ISO = new Date().toISOString().slice(0, 10);

const PROMPT_BASE = `You are extracting transport booking data (flight ticket, train ticket, bus ticket, boarding pass, e-ticket) from one or more files (PDF / image). The files belong to the SAME single booking — merge info you find across them.

CRITICAL — MULTI-SEGMENT (LAYOVERS / CONNECTIONS):
A booking can contain MULTIPLE legs / flights / trains with intermediate stops. The "Itinerary Details" table or list usually has one ROW per leg. You MUST return EACH physical leg as a SEPARATE item in the "segments" array, in chronological order, with its OWN distinct from_address, to_address, departure & arrival times.

Examples of how to split correctly:
- Flight Madrid → Belgrade → Moscow:
    segments[0] = { from_address: "Madrid (MAD)", to_address: "Belgrade (BEG)", departure_date/time of leg 1, arrival_date/time of leg 1 }
    segments[1] = { from_address: "Belgrade (BEG)", to_address: "Moscow (SVO)", departure_date/time of leg 2, arrival_date/time of leg 2 }
  NEVER write segments[1].from_address = "Madrid" — the second leg starts where the first one ARRIVED.
- Train Paris → Lyon → Marseille with a change: same logic — leg 1 ends in Lyon, leg 2 starts in Lyon.

VERIFICATION RULE: For every pair of consecutive segments, segments[i+1].from_address MUST equal (city or airport code of) segments[i].to_address. If you cannot satisfy this, you've misread the ticket — re-read it.

If the booking is direct / non-stop, return exactly ONE segment.

Common fields (booking_url, booking_platform) go at the top level — they apply to all segments. Carrier may differ per leg (e.g. codeshare); fill carrier per-segment. For total "price"/"currency", if only a grand total is shown, put it on the FIRST segment only and leave price empty on the others.

Return ONLY fields you can confidently read. Leave unknown fields empty. Detect the booking platform from logos/headers/footers/URLs. Times in 24h format. Currency as ISO 4217. For airports/stations include the IATA/station code + city in the address (e.g. "Madrid (MAD) Terminal 1").
...
- If that resulting date is already in the PAST (earlier than today), advance the year by 1 (use ${CURRENT_YEAR + 1} instead) — tickets are for the future, not the past.
- If a year IS explicitly written in the document, always honor it as-is (even if it's in the past — the user may be archiving an old ticket).`;

export default function TransferAiUpload({ onExtract, onCancel }) {
  ...
  const runLlm = async ({ fileUrls, urlForPrompt }) => {
    const result = await base44.integrations.Core.InvokeLLM({
      prompt: urlForPrompt
        ? `${PROMPT_BASE}\n\nAlso parse this booking URL: ${urlForPrompt}`
        : PROMPT_BASE,
      file_urls: fileUrls,
  ...
  return (
    <div className="rounded-xl border border-primary/30 bg-gradient-to-br from-primary/5 via-primary/[0.02] to-accent/5 p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-medium text-sm">{t('ai.transfer_title')}</h3>
      ...
      <div className="flex gap-2 mb-3">
        <div className="relative flex-1">
          ...
          <Input
            ...
            placeholder={t('ai.transfer_url_placeholder')}
            className="pl-9"
            disabled={busy}

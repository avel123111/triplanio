 ...
 *
 * ★ ЗАЧЕМ ЭТО ПОНАДОБИЛОСЬ. У поверхности с ручкой тап по ручке — её
 * собственный жест, и ради него `touchend` гасит эмулированный клик. Пока под
 * ручкой стоял один заголовок, это было безобидно; как только туда легло
 * УПРАВЛЕНИЕ, гашение клика убило его на телефоне: тап по кнопке срабатывал как
 * жест поверхности. Дефект поведенческий, скриншота у него нет, и в браузере он
 * выглядит как «кнопка не нажимается» — то есть как что угодно.
 * ⚠️ ШИТ ЭТИМ ПРАВИЛОМ БОЛЬШЕ НЕ ПОЛЬЗУЕТСЯ (TRIP-535): шелл кладёт в его шапку
 * прогресс, сброс и вкладки, и любой промах мимо кнопки читался как «переключи
 * детент». Тап по шапке шита принадлежит СОДЕРЖИМОМУ; тяга осталась. Читатель
 * обоих экспортов сегодня один — стопка тостов, где ручка и есть ручка.
 */
 ...
 *
 * ★ ТЯГА И ТАП РАЗВЕДЕНЫ НАМЕРЕННО. Потянуть поверхность за ручку можно откуда
 * угодно, включая кнопку: палец уже поехал, и это однозначно её жест
 * (`gestureOwner` выше про это). А вот ТАП по кнопке — это нажатие кнопки, и
 * поверхности он не принадлежит ни при каких условиях. Одно `onHandle` на оба
 * случая и было ошибкой: оно склеивало два разных намерения в одно.
 *

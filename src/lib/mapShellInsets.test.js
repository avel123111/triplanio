...
test('★★ ПОКА ИДЁТ ЖЕСТ — крайний размер карты, полосе фона взяться неоткуда', () => {
  // Высота приходит только на осадке, а шит едет за пальцем: карта старого
  // размера + уехавший вниз шит = полоса фона до 414 px на всё время движения.
...
test('★ у слота ровно ДВА размера, сколько бы ни было детентов', () => {
  // Отсюда и «один ресайз на жест»: холст переставляется между низом и
  // серединой, всё остальное движение шита для него не существует.

...
/**
 * ВЫСОТА, КОТОРУЮ ШИТ ОТРЕЗАЕТ У СЛОТА КАРТЫ. Не равна высоте шита, и в этом
 * весь смысл (TRIP-484).
 *
 ...
 * его вниз — карта ещё старого размера, и между её низом и верхом шита
 * открывается полоса фона на всё время движения: замер на экране 800 давал до
 * 414 px. Отдать холсту сразу крайний размер дешевле любого слежения: ресайз
 * один на жест (каждый кадр = переаллокация GL-буфера), а шит просто съезжает
 * по уже нарисованной карте.
 *
 * ★★ НА ОСАДКЕ — НЕ ВЫШЕ СРЕДНЕГО ДЕТЕНТА. Движение шита с середины вверх
 * означает, что он закроет экран целиком: карту под ним двигать незачем, её
 * никто не увидит. Заодно исчезает состояние «холст в 20 px» на верхнем
 * детенте — то самое, в которое уходил и в котором застревал автофокус.
 *
 * @param {{ sheetPx?: number, stops?: number[], dragging?: boolean }} [p]
 *   `stops` — высоты детентов по возрастанию (их считает сам шит).
 * @returns {number}
 */
export function mapSlotPx({ sheetPx = 0, stops = [], dragging = false } = {}) {
  ...
  const cap = px(list[Math.max(0, list.length - 2)]);
  return Math.min(px(sheetPx), cap);
}

...
//
// ★ ЗАЧЕМ ОТДЕЛЬНЫЙ МОДУЛЬ. Предикаты «городу нужен отель» / «на стыке нет
// переезда» нужны И ленте, и этому виджету. Вторая копия разъехалась бы молча:
// варнинг в ленте есть, а прогресс его не считает — этого не видит ни один гард.
//
// ★★ ЗНАМЕНАТЕЛЬ СЧИТАЕТСЯ ОТ МАРШРУТА, А НЕ ОТ БРОНЕЙ: идём по узлам и
// спрашиваем «покрыт ли этот ночлег / этот стык». Отсюда САМО СОБОЙ следует
// «отмершие брони не учитываются» — переезд между городами, которые больше не
// соседи, не покрывает ни одного стыка и в обходе просто не встречается.
//
...
// Переезды ровно с этого узла на этот. Наружу не выходит: единственный читатель
// — `buildPreparation` ниже, а «покрыт ли стык» вызыватели спрашивают у неё.

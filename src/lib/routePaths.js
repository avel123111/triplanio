 ...
 *   · `/`, `/d/…`      — у страницы ЕСТЬ языковые адреса, и беспрефиксный из
 *                        них английский (обычная роль `defaultLocale`);
 *   · всё остальное    — `null`: у страницы ОДНА версия, адрес про язык молчит,
 *                        и решает посетитель.
 *
 * ПОЧЕМУ ЭТО ВАЖНО ДО БУКВЫ. Первая редакция спрашивала здесь
 * `PRERENDERED_PAGES`, то есть отвечала «английский» ещё и на `/terms` и
 * `/privacy`. У них языкового адреса нет и быть не может (документ английский по
 * решению TRIP-465 §7) — но ОБВЯЗКА у них переведена, и переключатель языка на
 * них стоит. Выходило, что адрес обязуется показать английский там, где
 * английского адреса не существует: человек переключал язык на `/terms`,
 * нажимал вкладку «Privacy» — и обвязка молча возвращалась в английскую, потому
 * что смена пути переспрашивала этот предикат.
 *
 * @param {string} pathname

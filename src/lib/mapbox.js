...
export function clampPadding(map, padding = 0) {
  ...
  // противоположных сторон обязана оставить полосу канваса, иначе
  // `cameraForBounds` молча отказывает. Именно этим отступом выражается площадь,
  // закрытая панелью, поэтому объект здесь не экзотика, а основной случай.

...
/**
 * Записать черновик, СОХРАНИВ метку передачи, если она уже стоит.
 *
 * ★ ИМЕННО СОХРАНИВ, И ЭТО НЕ ПЕРЕСТРАХОВКА. Метку ставит `markHandoff` в один
 * момент — когда человек уходит регистрироваться, — а пишет черновик эффект
 * персистенции, на каждое изменение состояния. Оба пишут ОДИН ключ, и запись
 * без метки её стирает. Сегодня это не срабатывает лишь потому, что между
 * нажатием и уходом со страницы ни одна зависимость эффекта не успевает
 * измениться. Любая поздняя правка состояния в это окно (доехал
 * `resolveCities` и обновил города, пересчиталась цепочка дат) молча снимает
 * метку — и человек возвращается после регистрации к ПУСТОМУ планировщику,
 * то есть ровно к тому отказу, ради которого модуль и написан. Ни гардом, ни
 * глазами это не видно: обе записи валидны.
 *
 * Тратит метку только `takeHandoff` — тот, кто её получил.
 *
 * @param {string|null|undefined} userId
 * @param {string} method
 * @param {Record<string, any>} draft

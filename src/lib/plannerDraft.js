...
/**
 * Ждёт ли переданный гостевой черновик того, кто войдёт (TRIP-505).
 *
 * Нужен экрану входа, чтобы объяснить человеку, зачем его сюда привели
 * («маршрут уже собран — войдите, и он сохранится»). Предикат именно такой, а
 * НЕ «адрес возврата — планировщик»: обещание в подписи касается МАРШРУТА, и
 * честно его давать только когда маршрут действительно отложен. Адрес возврата
 * мог остаться от другого пути (приглашение, адрес приложения без сессии), и
 * подпись про маршрут была бы там ложью.
 *
 * Ничего не тратит и не двигает — только смотрит.
 *
 * @param {string} method
 * @param {number} now
 * @returns {boolean}
 */
export function hasPendingHandoff(method, now) {
  return parseDraft(get(draftKey(GUEST_ID, method)), now)?.handoff === true;
}

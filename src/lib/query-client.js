...
// Карточки трипов (`['trip-card', id]`) кладёт в кэш ГЛАВНАЯ, а читает их ДРУГОЙ
// экран — трип. Между этими моментами у записи нет ни одного наблюдателя, а
// react-query вычищает такие записи по `gcTime` (дефолт — 5 минут). То есть с
// дефолтом первый кадр меню собирался бы из фактов только у того, кто кликнул
// карточку сразу; отвлёкся на десять минут — и всё снова доезжает на глазах.
// Дефолт задаётся на СЕМЕЙСТВО ключей здесь, рядом с остальной политикой кэша, а
// не в месте записи: срок жизни данных — свойство данных, а не вызова.
// 30 минут — та же величина, что у соседнего долгоживущего кэша Pro-статуса.
queryClientInstance.setQueryDefaults(['trip-card'], { gcTime: 30 * 60 * 1000 });

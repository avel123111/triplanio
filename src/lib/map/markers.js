...
// ═══════════════════════════════════════════════════════════════════════════
// ВЕС МЕТОК ЗАДАЁТСЯ ПОД РАЗМЕР, В КОТОРОМ НА ПОВЕРХНОСТЬ СМОТРЯТ (Pavel)
//
// Канон `.tmk` (29 px кольцо) и канон линии (3.5 px) нарисованы под живую карту:
// её смотрят 1:1, там пиксель макета = пиксель на экране. Карта share-карточки
// живёт иначе — её РЕНДЕРЯТ крупнее, чем на неё потом смотрят: слот 810 px, а
// сторис на телефоне показывается шириной ~390. Те же 29 px весят там вдвое
// меньше, и это НЕ вопрос зум-диапазона: даже на полном размере пин занимал 3.6%
// ширины карты против 6.5% в приложении, а линия — 0.43% против 0.90% (диапазон
// линий вообще не касается). Замер по кадру «Классическая Италия».
//
// Поэтому правило одно на все поверхности и звучит так: **метка рисуется под тот
// размер, в котором поверхность будут РАССМАТРИВАТЬ.** У живой карты это 1:1 —
// множитель 1, ничего не меняется. У карточки полотно шире эталона во столько-то
// раз — во столько же раз тяжелее метка. Это не второй дизайн и не вторые веса:
// облик тот же, меняется только масштаб под размер полотна.
//
// Эталон = карта в приложении на телефоне (~420 px), где канон и выглядит так,
// как задуман. Для слота стори (810) выходит ×1.93, для поста (866) — ×2.06;
// пин 56 px = 6.9% ширины карты против 7.4% в приложении, линия 6.8 px = 0.83%
// против 0.90%. То есть паритет, а не «покрупнее на глаз».
export const MARKER_REF_WIDTH = 420;

/**
 * Множитель веса меток для полотна шириной `renderWidth` (в пикселях ФИНАЛЬНОГО
 * рендера, не превью). Ниже эталона не опускается: узкое полотно — повод
 * оставить канон, а не измельчать метки до нечитаемости.
 * @param {number} renderWidth
 */
export function markerSurfaceWeight(renderWidth) {
  return Math.max(1, (renderWidth || 0) / MARKER_REF_WIDTH);
}
 ...
 *
 * `weight` — вес метки под размер полотна (`markerSurfaceWeight`). Он множит
 * ТОЛЬКО амплитуду: сдвиг стопов отвечает на вопрос «какой зум у финала» и от
 * веса не зависит — смешать их значило бы двигать кадр вместе с размером.
 * @param {number} s
 * @param {number} weight
 */
export function markerZoomSizeExpr(s = 1, weight = 1) {
  const { MIN, MAX, Z_LO, Z_HI } = MARKER_ZOOM_SCALE;
  const shift = Math.log2(s > 0 ? s : 1); // s<1 ⇒ отрицательный: полки ниже по зуму
  const k = s * weight;
  return ['interpolate', ['linear'], ['zoom'], Z_LO + shift, MIN * k, Z_HI + shift, MAX * k];
}

...
export default function MapControls({ projection, onToggleProjection, scheme, onToggleScheme, showSE, onToggleSE }) {
  ...
  const buttons = [
    { key: 'proj', label: t('tse.map_proj_aria'), title: projection === 'globe' ? t('tse.map_flat') : t('tse.map_globe'), icon: projection === 'globe' ? 'map' : 'globe', onClick: onToggleProjection, on: projection === 'globe' },
    { key: 'theme', label: t('tse.map_light_aria'), title: scheme === 'DARK' ? t('tse.map_light') : t('tse.map_dark'), icon: scheme === 'DARK' ? 'sun' : 'moon', onClick: onToggleScheme, on: scheme === 'LIGHT' },
    { key: 'se', label: t('tse.map_se_aria'), title: t('tse.map_startend'), icon: showSE ? 'flag' : 'eyeOff', onClick: onToggleSE, on: showSE },
  ];
  return (

...
/**
 * «Это один и тот же город» — ЕДИНСТВЕННЫЙ способ сравнить два узла;
 * `cityIdentity(a) === cityIdentity(b)` напрямую больше нигде не пишем.
 *
 * ★★ ПУСТАЯ ИДЕНТИЧНОСТЬ НЕ РАВНА ПУСТОЙ: `''` значит «не знаем, какой это
 * город», а не «тот же». Сравнение в лоб делало два неопознанных узла одним и
 * молча гасило варнинг «Нет переезда» и стыки в «Подготовке».
 */
...
// ★★`blocks` - предикат «эта претензия РЕАЛЬНО не даёт сохранить в ЭТОЙ форме».
// По умолчанию блокирует любая ошибка, и для форм на `useHybridValidation` это
// верно: их `canSubmit` смотрит ровно на `level === 'error'`. Но редактор события
// применяет свою политику вердикта - понижает до совета всё, чего нет в его
// списке `BLOCKING_CODES` (это НЕ «всё, что кончается на `_ORDER`»: `SEG_BACKSTEP`
// в списке есть, а `INS_DATE_ORDER`/`CITY_ORDER` - нет), - и без этого параметра
// звёздочка врала бы именно там: у аренды авто название помечено обязательным, а
// форма сохраняется без него. Обязательность обязана считаться по ТОМУ ЖЕ гейту,
// который держит кнопку сохранения, иначе это снова два источника правды.
export function isFieldRequired(kind, draft = {}, ctx = {}, field, blocks = (i) => i.level === 'error') {
  if (!field) return false;
  const probe = blankField(draft, field);
  return validateEntity(kind, probe, ctx)
    .some((i) => i.field === field && blocks(i));
}

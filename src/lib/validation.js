...
// применяет свою политику вердикта - понижает всё, кроме семейства *_ORDER, до
// совета (см. BLOCKING_CODES там же), - и без этого параметра звёздочка врала бы
// именно там: у аренды авто название помечено обязательным, а форма сохраняется
// без него. Обязательность обязана считаться по ТОМУ ЖЕ гейту, который держит
// кнопку сохранения, иначе это снова два источника правды.

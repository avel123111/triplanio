...
test('десктоп: карта заходит под скругление панели', () => {
  assert.deepEqual(mapSlotInsets({ panelPx: 550 }), { bottom: 0, left: 530 });
});

test('десктоп: заход под скругление не уводит слот в минус', () => {
  assert.deepEqual(mapSlotInsets({ panelPx: 12 }), { bottom: 0, left: 0 });
});
...
test('режимы не смешиваются: чужая величина не читается', () => {
  ...
  assert.deepEqual(mapSlotInsets({ phone: false, sheetPx: 612, panelPx: 550 }), { bottom: 0, left: 530 });

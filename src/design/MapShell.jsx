 ...
 *
 * ★★ КАМЕРУ ШЕЛЛ НЕ ТРОГАЕТ САМ — он лишь СЧИТАЕТ свободное место и отдаёт его
 * вызывателю (`map(insets)`). Как им распорядиться, решает карта, и решение это
 * НЕ произвольное: отступ вьюпорта на проекции `globe` роняет зум и
 * рассинхронивает лимб атмосферы с тайлами (замерено на живой карте — снимки в
 * PR), поэтому закрытая площадь обязана выражаться СДВИГОМ камеры, а не
 * отступом.
 *

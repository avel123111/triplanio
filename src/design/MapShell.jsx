 ...
 *
 * ★ КАРТА НЕ МЕНЯЕТ РАЗМЕР — МЕНЯЕТСЯ ПРИЦЕЛ КАМЕРЫ. Слот карты всегда во всю
 * площадь шелла; ни панель, ни шит его не режут. Резать канвас значит
 * переаллоцировать буфер и перерисовывать тайлы — это и есть «карта моргнула /
 * пропала на мгновение». Вместо этого шелл СЧИТАЕТ закрытую площадь в пикселях
 * и отдаёт её карте отступами (`insets`), а та целится в остаток.
 *
 * ★ ПЕРЕКАДРИРОВАНИЕ — ПОСЛЕ ФИКСАЦИИ, НЕ ВО ВРЕМЯ ЖЕСТА. Шит сообщает высоту
 * ЗАФИКСИРОВАННОГО детента; пока палец ведёт шит, камера не трогается — иначе
 * она дерётся с жестом.
 *

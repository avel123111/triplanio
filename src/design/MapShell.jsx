...
export function MapShell({
  ...
  // ★ «ОКНО ИЗВЕСТНО» — ОТДЕЛЬНЫЙ ФАКТ, И БЕЗ НЕГО КАДРИРОВАТЬ НЕЛЬЗЯ. Тот же
  // закон, что у `canFit` («не вписывай в неизмеренный холст»), только про
  // ВТОРУЮ величину: свободное окно. Шит сообщает свою кромку СВОИМ эффектом, а
  // эффекты карты объявлены выше по дереву и успевают отработать раньше —
  // значит первый фит штатно приходится на момент, когда закрытая площадь ещё
  // ноль. Кадр тогда считается по ВСЕМУ холсту, а перекадрирования не будет
  // (автофокус — только на изменение маршрута), и маршрут остаётся в четыре раза
  // крупнее окна НАВСЕГДА. Замер на стенде (кромка задержана): пины −215 и +433
  // при окне 0..246, то есть одна точка под шапкой, вторая под шитом — ровно то,
  // что видел Pavel (`y-170,376` с его устройства).
  // Ждать нечего там, где шита нет вовсе: десктоп и шелл без панели.
  const measured = !isPhone || !panel || coveredPx > 0;
  const view = useMemo(() => ({ camera: box.camera, fit: box.fit, measured }), [box, measured]);

 ...
 * ⚠️ Кавычки поля читаются ОБЕ: словарь от `'` к `"` не сломается ничем (прогон
 * форматтера, другой файл), а сверка молча перестала бы видеть весь этот класс
 * имён — то есть ровно то «пустое место без предупреждения», против которого
 * гард и заведён.

...
// лентой с нативным `scroll-snap`. Mobile-first — на тач-устройстве это просто
// свайп-скролл; на десктопе (`@media (hover:hover)`) появляются кнопки-стрелки
// ◄►, листающие на ~страницу, и видны ТОЛЬКО когда в ту сторону есть куда ехать.
  ...
  ({ children, className = "", ariaLabel, prevLabel, nextLabel, ...rest }, ref) => {
    ...
    return (
      <div className={["carousel", className].filter(Boolean).join(" ")} ref={ref}>
        <div
          className="carousel__track scrollbar-thin"
          ref={trackRef}
          onScroll={sync}
          role="group"

...
test('примитивы эмитят СВОЙ класс и пробрасывают className', () => {
  ...
  // ⚠️ Рантайм их пропускает, ТИП — нет; граница названа в шапке Layout.jsx.
  for (const comp of ['Row', 'Col', 'Grid', 'Trunc', 'Grow']) {
...
test('★ тип НЕ запечатывает набор пропов: DOM-атрибуты обязаны проезжать', () => {
  ...
  for (const comp of ['Row', 'Col', 'Grid', 'Trunc', 'Grow']) {
    ...
    assert.match(decl, /ComponentPropsWithoutRef<'div'>/, `${comp} объявлен без DOM-атрибутов — экран под @ts-check упрётся в TS2322`);
  }
});

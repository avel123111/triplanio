...
import { CANONS, SANCTIONED_MODS, MODIFIERS, COLORS, probeCanons, detectCanon, comboApply, probeColors, detectColor, colorByKey } from './canons.js';
...
// Человекочитаемый лейбл модификатора по ключу (strong/flush/caption).
...
function render(el) {
  ...
  // modifiers — РОВНО три санкционированных орто-модификатора (TRIP-410): t-strong /
  // t-flush / tp-caption. Комбинируются с любым каноном; эфемерное превью на элементе.
  const mods = pending ? SANCTIONED_MODS : [];
...
function pickCanon(el, id) {
  ...
  previewMod.delete(el);                   // модификатор-превью тоже сбрасываем при смене канона
  applyPreview(el);
  render(el);
}
// Санкционированный модификатор (TRIP-410) — single-select эфемерное превью,
...
// Inline CSS активного санкционированного модификатора (слой поверх канона).
function modCssFor(el) {
  const idx = previewMod.get(el);
  const pending = pendingCanon.get(el);
  if (idx == null || !pending) return null;
  return SANCTIONED_MODS[idx]?.css || null;
}
